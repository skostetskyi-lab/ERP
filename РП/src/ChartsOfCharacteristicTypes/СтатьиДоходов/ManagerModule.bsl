...
Процедура ЗаполнитьПредопределенныеСтатьиДоходов() Экспорт
	...
	//++ НЕ УТ
	СтатьиДоходовОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ДоходОтОперационнойКурсовойРазницы;
	//-- НЕ УТ
	Попытка
		СтатьиДоходовОбъект.Записать();
	Исключение
	...
	СтатьиДоходовОбъект.ТипЗначения = Новый ОписаниеТипов("ПеречислениеСсылка.АналитикаКурсовыхРазниц");
	//++ НЕ УТ
	СтатьиДоходовОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ДоходОтНеоперационнойКурсовойРазницы;
	//-- НЕ УТ
	Попытка
		СтатьиДоходовОбъект.Записать();
	Исключение
	...
	СтатьиДоходовОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ДоходОтРеализацииДругихОборотныхАктивов;
	//-- НЕ УТ
	Попытка
		СтатьиДоходовОбъект.Записать();
	Исключение
	...
	//++ НЕ УТ
	СтатьиДоходовОбъект = ПланыВидовХарактеристик.СтатьиДоходов.ДоходыПриПокупкеИПродажеИностраннойВалюты.ПолучитьОбъект();
	СтатьиДоходовОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ДоходОтРеализацииИностраннойВалюты;
	Попытка
		СтатьиДоходовОбъект.Записать();
	Исключение
	КонецПопытки;
	
	СтатьиДоходовОбъект = ПланыВидовХарактеристик.СтатьиДоходов.РазницыСтоимостиВозвратаИФактическойСтоимостиТоваров.ПолучитьОбъект();
	СтатьиДоходовОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ДругиеДоходыОтОперационнойДеятельности;
	Попытка
		СтатьиДоходовОбъект.Записать();
	Исключение
	КонецПопытки;
	
	СтатьиДоходовОбъект = ПланыВидовХарактеристик.СтатьиДоходов.ЗакрытиеРезервовПоСомнительнымДолгам.ПолучитьОбъект();
	СтатьиДоходовОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ДругиеДоходыОтОперационнойДеятельности;
	Попытка
		СтатьиДоходовОбъект.Записать();
	Исключение
	...
	СтатьиДоходовОбъект = ПланыВидовХарактеристик.СтатьиДоходов.ДоходыПриВосстановленииНалоговогоКредита.ПолучитьОбъект();
	СтатьиДоходовОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ДругиеДоходыОтОперационнойДеятельности;
	Попытка
		СтатьиДоходовОбъект.Записать();
	Исключение
	...
	СтатьиДоходовОбъект = ПланыВидовХарактеристик.СтатьиДоходов.ОкруглениеНаличныхПриРозничныхПродажах.ПолучитьОбъект();
	СтатьиДоходовОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ДругиеДоходыОтОперационнойДеятельности;
	Попытка
		СтатьиДоходовОбъект.Записать();
	Исключение
...
Процедура ЗаполнитьСтатьяДоходовОкруглениеНаличныхПриРозничныхПродажах() Экспорт
	
	Объект = ПланыВидовХарактеристик.СтатьиДоходов.ОкруглениеНаличныхПриРозничныхПродажах.ПолучитьОбъект();
    //++ НЕ УТ
    Объект.СчетУчета = ПланыСчетов.Хозрасчетный.ДругиеДоходыОтОперационнойДеятельности;
	//-- НЕ УТ
