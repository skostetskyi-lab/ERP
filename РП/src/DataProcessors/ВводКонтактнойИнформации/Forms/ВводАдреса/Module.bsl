...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), 
		Строка(ВидКонтактнойИнформации.Ссылка),
		Параметры.Заголовок);
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформации(Контекст, РедактируемаяИнформация)
	...
	Если УправлениеКонтактнойИнформациейСлужебный.ЭтоНациональныйАдрес(ДанныеАдреса) Тогда
		...
		ЗначениеРеквизитовПоКонтактнойИнформацииАдресНациональный(ДанныеАдреса, Контекст);
	Иначе
...
&НаСервереБезКонтекста                   
Процедура ЗначениеРеквизитовПоКонтактнойИнформацииАдресНациональный(ДанныеАдреса, Контекст)
...
Функция КонтактнаяИнформацияПоЗначениюРеквизитов(Контекст)
	...
	Адрес.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()));
	...
	Если Контекст.НаселенныйПунктДетально <> Неопределено Тогда
		Для Каждого КлючЗначение Из Контекст.НаселенныйПунктДетально Цикл
			...
			УправлениеКонтактнойИнформациейСлужебный.УстановитьXDTOРеквизитОбъекта(АдресНациональный, ПутьXPath, ЧастьПункта.Представление);
		КонецЦикла;
	КонецЕсли;
	...
	УправлениеКонтактнойИнформациейСлужебный.ЗданияИПомещенияАдреса(АдресНациональный, 
		Новый Структура("Здания, Помещения", ТаблицаЗданий, ТаблицаПомещений));
	
	// Индекс                                                                        
	УправлениеКонтактнойИнформациейСлужебный.ПочтовыйИндексАдреса(АдресНациональный, Контекст.Индекс);
	
	//
	Если НЕ ПустаяСтрока( Контекст.ДополнительныйЭлемент) Тогда                             
		УправлениеКонтактнойИнформациейСлужебный.ДобавитьДополнительныеЭлементыАдреса(АдресНациональный, Контекст.ДополнительныйЭлемент, 90);
	КонецЕсли;
	Если НЕ ПустаяСтрока(Контекст.ПодчиненныйЭлемент) Тогда                                
		УправлениеКонтактнойИнформациейСлужебный.ДобавитьДополнительныеЭлементыАдреса(АдресНациональный, Контекст.ПодчиненныйЭлемент, 91);
	КонецЕсли;
	...
	Если Контекст.РазрешитьВводАдресаВСвободнойФорме И Контекст.ПредставлениеАдресаИзменено Тогда
		...
		Иначе
			...
			АдресНациональный.Адрес_по_документу = ВведенноеПредставление;
		КонецЕсли;
	Иначе
		АдресНациональный.Сбросить("Адрес_по_документу");
		Результат.Представление = РасчетноеПредставление;
...
Процедура ЗаполнитьКонтекстПоXDTOАдреса(XDTOДанныеАдреса, Контекст, Уровни = Неопределено)
	...
	// Из переданного XDTO   
	АдресНациональный = УправлениеКонтактнойИнформациейСлужебный.НациональныйАдрес(XDTOДанныеАдреса);
		...
		Для Каждого КлючЗначение Из Контекст.НаселенныйПунктДетально Цикл
			...
			Если Уровни = Неопределено ИЛИ Уровни.Найти(Значение.Уровень) <> Неопределено Тогда
				Если Значение.Уровень <> 90 Тогда
					Представление = УправлениеКонтактнойИнформациейСлужебный.ПолучитьXDTOРеквизитОбъекта(АдресНациональный, Значение.ПутьXPath);
				Иначе
					Представление = УправлениеКонтактнойИнформациейСлужебный.НайтиДополнительныйЭлементАдреса(АдресНациональный);
				КонецЕсли;
...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС, ДействуетДо)
	...
	Если ЗначениеЗаполнено(ДействуетДо) И ЗначениеЗаполнено(ДействуетС)Тогда
		...
		Иначе
			ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адрес действовал с %1 по %2 (включительно)';uk='Адреса діяла з %1 по %2 (включно)'"), Формат(ДействуетС, "ДЛФ=DD"), Формат(ДействуетДо - 10, "ДЛФ=DD"));
		КонецЕсли;
...
Функция ПутьКДаннымФормыПоПутиXPath(ПутьXPath) 
	...
	Если ПутьXPath = "Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный() Тогда
