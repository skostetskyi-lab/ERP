...
Процедура ВыполнитьКорректировкуНаСервере(ДанныеДляИсправления)
	Попытка
		...
		Документ.Комментарий =НСтр("ru='Исправление периода военного сбора по документу';uk='Виправлення періоду військового збору за документом'") + " " + Локализация.ПолучитьЛокализованноеПредставление(ДанныеДляИсправления.Документ, Локализация.КодЯзыкаИнформационнойБазы());
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Создано документ';uk='Створено документ'")+ " "+Локализация.ПолучитьЛокализованноеПредставление(Документ, Локализация.КодЯзыкаИнтерфейса()));
