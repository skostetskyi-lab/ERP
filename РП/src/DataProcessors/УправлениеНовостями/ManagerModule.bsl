...
Функция ПодключитьсяКВебСервисуОбновленияСтандартныхСписков()
	...
	ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
		НСтр("ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Начало';uk='Новини. Завантаження класифікаторів. З''єднання з вебсервісом оновлення стандартних списків. Початок'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
		ТекстСообщения // Комментарий
	...
	Попытка
		...
		Если ТипЗнч(Прокси) <> Тип("WSПрокси") Тогда
			ТекстСообщения = НСтр("ru='Неудачное подключение к веб-сервису обновления стандартных списков';uk='Невдале підключення до вебсервісу оновлення стандартних списків'");
			// Запись в журнал регистрации
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Ошибка';uk='Новини. Завантаження класифікаторів. З''єднання з вебсервісом оновлення стандартних списків. Помилка'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
		...
		Иначе
			ТекстСообщения = НСтр("ru='Успешное подключение к веб-сервису обновления стандартных списков';uk='Успішне підключення до вебсервісу оновлення стандартних списків'");
			// Запись в журнал регистрации
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Загрузка классификаторов';uk='БІП:Новини.Завантаження класифікаторів'"), // ИмяСобытия
				НСтр("ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков. Конец';uk='Новини. Завантаження класифікаторів. З''єднання з вебсервісом оновлення стандартних списків. Кінець'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения // Комментарий
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru='Неудачное подключение к веб-сервису обновления стандартных списков по причине:
            |%ИнформацияОбОшибке%'
            |;uk='Невдале підключення до вебсервісу оновлення стандартних списків з причини:
            |%ИнформацияОбОшибке%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		// Запись в журнал регистрации
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
