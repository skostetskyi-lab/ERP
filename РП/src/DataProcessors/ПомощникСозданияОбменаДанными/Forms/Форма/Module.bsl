...
Процедура ПроверитьПодключение(ВидТранспорта)
	...
	Если Не Отказ Тогда
		...
		Если ВидТранспорта = ПредопределенноеЗначение("Перечисление.ВидыТранспортаСообщенийОбмена.FILE") Тогда
			ТекстПредупреждения = НСтр("ru='Запись данных в указанный каталог разрешена.';uk='Запис даних у вказаний каталог дозволений.'");
		Иначе
...
Функция Подключаемый_СтраницаМастераНачало_ПриПереходеДалее(Отказ)
	...
	ТекстПредупреждения = НСтр("ru='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента платформы.';uk='Для передачі файлу налаштувань синхронізації даних необхідно встановити розширення для вебклієнта платформи.'");
