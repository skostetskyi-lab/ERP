...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеКонфигурацииИсточника = ?(Объект.КонфигурацияИсточник = "УТП",
	  	...
	  	НСтр("ru='""Управление производственным предприятием для Украины"", ред.1.3';uk='""Управління виробничим підприємством для України"", ред. 1.3'")
    );
...
Процедура ПриОткрытии(Отказ)
    ...
    Если Объект.МетаданныеИмя <> "BASERP" И Объект.МетаданныеИмя <> "BASКомплексноеУправлениеПредприятием" Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Данная обработка предназначена для запуска в программе 
            |""BAS ERP"", ред.2.1 или ""BAS Комплексное управление предприятием"", ред.2.1'
            |;uk='Ця обробка призначена для запуску в програмі 
            |""BAS ERP"", ред. 2.1 або ""BAS Комплексне управління підприємством"", ред. 2.1'"));
		Отказ = Истина;
	КонецЕсли;
	...
	Если АвтоТестОбмена Тогда
		#Если ВебКлиент Тогда
			ТекстСообщения = НСтр("ru='Автотест переноса данных не может выполняться в режиме Веб-клиента.';uk='Автотест перенесення даних не може виконуватися в режимі Вебклієнта.'");
...
Функция ПроверитьПодключениеКИБ()
	...
	Попытка
        ...
        Если ИмяКонфигурации = "УправлениеПроизводственнымПредприятиемДляУкраины" ИЛИ ИмяКонфигурации = "УправлениеТорговымПредприятиемДляУкраины" Тогда 
        
            ТекстСообщения    = СравнитьВерсии(ИБИсточник, ?(Объект.КонфигурацияИсточник = "УПП", "ПравилаОбмена", "ПравилаОбменаУТП"));
            ТекстСообщенияЗУП = СравнитьВерсии(ИБИсточник, ?(Объект.КонфигурацияИсточник = "УПП", "ПравилаОбменаЗУП", "ПравилаОбменаЗУПУТП"));
		...
		Иначе
			...
			Если РазделениеВключено Тогда
				...
				Если Найти(ОбъектОбработки.Значение, "УППУП_ОД") Тогда
					// Основные данные
                    ИмяМакета = ?(Объект.КонфигурацияИсточник = "УПП", "ПравилаОбмена", "ПравилаОбменаУТП");
				Иначе
					// ЗУП данные
                    ИмяМакета = ?(Объект.КонфигурацияИсточник = "УПП", "ПравилаОбменаЗУП", "ПравилаОбменаЗУПУТП");
				КонецЕсли;
...
Процедура ВыгрузкаДанныхВыполнить(Результат, ДополнительныеДанные) Экспорт
	...
	Иначе
        Если Итерация = 1 Тогда
            ИмяКонвертацияПравила = ?(Объект.КонфигурацияИсточник = "УПП", "ПравилаОбмена", "ПравилаОбменаУТП");
        Иначе // 2.
            ИмяКонвертацияПравила = ?(Объект.КонфигурацияИсточник = "УПП", "ПравилаОбменаЗУП", "ПравилаОбменаЗУПУТП");
		КонецЕсли;
