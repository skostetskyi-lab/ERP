...
Функция ЗакрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаОплат, ВыходныеПараметры, СуммаЧека = 0, НомерЧекаВозврата = 0, ЧекККМ = неопределено) Экспорт
	...
	Для каждого СтрокаОплаты Из ТаблицаОплат Цикл
		...
		ИначеЕсли СтрокаОплаты.ТипОплаты = 1 Тогда
			СуммаБезналичнойОплаты1 = СуммаБезналичнойОплаты1 + СтрокаОплаты.Сумма;
		ИначеЕсли СтрокаОплаты.ТипОплаты = 3 Тогда
			СуммаБезналичнойОплаты3 = СуммаБезналичнойОплаты3 + СтрокаОплаты.Сумма;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ЗначениеЗаполнено(СуммаБезналичнойОплаты1) ИЛИ ЗначениеЗаполнено(СуммаБезналичнойОплаты2) ИЛИ ЗначениеЗаполнено(СуммаБезналичнойОплаты3) Тогда
		
		Попытка
			
			ОбъектДрайвера.УстановитьРасширенныйПорядокОплатыЧека(ПараметрыПодключения.ИДУстройства, 
				0, , , , , 
				СформироватьТаблицуДопРеквизитовОплаты(ТаблицаОплат));
...
Функция СформироватьШаблонЧека(ВходныеПараметры, ДополнительныйТекст = Неопределено, ТипОборудования = "") Экспорт
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Для ИндексОплаты = 0 По ТаблицаОплат.Количество() - 1 Цикл
			...
			Иначе
				СтрокаОплаты.Вставить("Код");
				СтрокаОплаты.Вставить("ДополнительныеСвойства");			
			КонецЕсли;
	...
	Если Результат Тогда
		...
		Если НЕ ОшибкаПриПечати Тогда
			...
			Иначе
				НомерЧекаВозврата = "";
			КонецЕсли;	                                                
			Результат = ЗакрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаОплат, ВыходныеПараметры, СуммаЧека, НомерЧекаВозврата, ОбщиеПараметры.ДокументОснование);
		Иначе
			Результат = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьТаблицуДопРеквизитовОплаты(ТаблицаОплат) Экспорт 
	...
	Для каждого СтрокаОплаты Из ТаблицаОплат Цикл
	    ...
	    ЭтоКарта = СтрокаОплаты.ТипОплаты = 1;
		
		Если ЭтоКарта И ЗначениеЗаполнено(СтрокаОплаты.ДополнительныеСвойства) Тогда
			
			ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("TerminalID", ""); 
			ЗаписьXML.ЗаписатьАтрибут("ApprovalCode", СтрокаОплаты.ДополнительныеСвойства.КодАвторизации); 
			ЗаписьXML.ЗаписатьАтрибут("RRN", СтрокаОплаты.ДополнительныеСвойства.СсылочныйНомер); 
			//ЗаписьXML.ЗаписатьАтрибут("IssuerName", ""));  
			ЗаписьXML.ЗаписатьАтрибут("PAN" , СтрокаОплаты.ДополнительныеСвойства.НомерПлатежнойКарты); 
			//ЗаписьXML.ЗаписатьАтрибут("TransactionDate" , ""); 
			//ЗаписьXML.ЗаписатьАтрибут("AcquireName" , ""); 
			ЗаписьXML.ЗаписатьАтрибут("InvoiceNumber" , СтрокаОплаты.ДополнительныеСвойства.НомерЧекаЭТ); 
			//ЗаписьXML.ЗаписатьАтрибут("ParentRRN" , "");     
			//ЗаписьXML.ЗаписатьАтрибут("SignVerif" , ""); 
			ЗаписьXML.ЗаписатьКонецЭлемента(); 
			
			//ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("CustomerEmail" , ""); 
			//ЗаписьXML.ЗаписатьАтрибут("AutosendCustomerEmail", ""); 
			//ЗаписьXML.ЗаписатьКонецЭлемента(); 
			//
			//ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("CustomerSMSNumber" , ""); 
			//ЗаписьXML.ЗаписатьАтрибут("AutosendCustomerSMS", ""); 
			//ЗаписьXML.ЗаписатьКонецЭлемента(); 
			//
			//ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("CustomerViberNumber" , ""); 
			//ЗаписьXML.ЗаписатьАтрибут("AutosendCustomerViber", ""); 
			//ЗаписьXML.ЗаписатьКонецЭлемента(); 
			//
			//ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("SumPayByBank", ""); 
			//ЗаписьXML.ЗаписатьКонецЭлемента(); 
			//
			//ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("BankPayToolsNM", "")); 
			//ЗаписьXML.ЗаписатьКонецЭлемента(); 
			//
			//ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("PrePaymentPayToolsNM", ""); 
			//ЗаписьXML.ЗаписатьКонецЭлемента(); 
			//
			//ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("AfterPaymentPayToolsNM", ""); 
			//ЗаписьXML.ЗаписатьКонецЭлемента(); 
			//
			//ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
			//ЗаписьXML.ЗаписатьАтрибут("UseTransactionDetailsToSumPayByBank", "True");
			//ЗаписьXML.ЗаписатьКонецЭлемента();
		...
		ЗаписьXML.ЗаписатьНачалоЭлемента("Record"); 
		ЗаписьXML.ЗаписатьАтрибут("PayFormCD" , "202");
		ЗаписьXML.ЗаписатьАтрибут("PayFormSum" , Формат(СтрокаОплаты.Сумма, "ЧРГ=.; ЧГ=")); 
		ЗаписьXML.ЗаписатьАтрибут("PayFormNM" , СтрокаОплаты.Наименование); 
		ЗаписьXML.ЗаписатьАтрибут("PayFormUseTransactionDetails" , ?(ЭтоКарта, "True", "False")); 
