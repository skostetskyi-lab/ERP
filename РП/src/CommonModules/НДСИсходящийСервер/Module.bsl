...
Процедура НайтиСтрокиНаСуммуПревышения(ВыборкаПоПревышениямНО, ТаблицаНоменклатурыДляЗаполнения, ТаблицаНоменклатурыНаСумму)
	...
	СтруктураПоиска = Новый Структура("АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта, ВидПоставки, ДокументПоставки");
...
Процедура ВыполнитьПроведениеДокументовПоВозникшемуНДС(НачальныеГраницы) Экспорт
	...
	СтарыеАналитики = Новый Структура("ПредоплатаАбсолютноеЗначение, КВозвратуАбсолютноеЗначение, ДолгАбсолютноеЗначение
										|, ЭтоПервоеСобытие, ЭтоОтгрузка, ЭтоОплата, ЭтоВозврат, ЭтоВозвратОплаты, ЭтоРозничныйПокупатель, РасчетныйДокументВозвратОплаты, РасчетныйДокументВозврат, ПоЗаявкеНаВозврат");
...
Функция ТекстЗапросаНалоговыхДокументов(НалоговаяНакладная = Истина, ОтборПоДокументам = Истина)
	...
	Если НалоговаяНакладная Тогда
        ...
        |			ИЛИ НалоговыйДокумент.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС14)
...
Процедура ПересчитатьНДСсУчетомПогрешностиОкругления(ТабличнаяЧасть, КлючДанных, ЦенаВключаетНДС, ПогрешностиОкругления, ПредставлениеТабличнойЧасти = "", ПредставлениеВалюты = "", 
	...
	Если СуммаНДСПослеОкругления <> СуммаНДСДоОкругления Тогда
		...
		ШаблонТекстаСообщения = НСтр("ru='%1 табличной части %2 изменилась (было %3 %4, стало %5 %4)';uk='%1 табличної частини %2 змінилася (було %3 %4, стало %5 %4)'");
