...
Процедура ПередатьФайлыНаСервер(ОповещениеЗавершения, Знач ИменаФайлов, Знач ИдентификаторФормы = Неопределено, Знач ТекстПредупреждения = Неопределено) Экспорт
	...
	Если ТекстПредупреждения = Неопределено Тогда
		Если ИменаФайлов.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("ru='Для передачи файла на сервер необходимо установить расширение для веб-клиента платформы.';uk='Для передачі файлу на сервер необхідно встановити розширення для вебклієнта платформи.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Для передачи файлов на сервер необходимо установить расширение для веб-клиента платформы.';uk='Для передачі файлів на сервер необхідно встановити розширення для вебклієнта платформи.'");
		КонецЕсли;
	КонецЕсли;
