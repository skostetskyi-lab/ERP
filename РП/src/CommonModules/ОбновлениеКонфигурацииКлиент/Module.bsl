...
Функция ПроверитьЛегальностьСкачиванияОбновления(ПараметрыЗапроса) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru='Ошибка создания описания веб-сервиса проверки легальности получения обновления.';uk='Помилка створення опису вебсервіса перевірки легальності одержання оновлення.'");
...
Функция ОписаниеСервисаПроверкиЛегальности()
	...
	Исключение
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса.
            ...
            |;uk='Помилка при створенні опису вебсервіса.
	...
	Исключение
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |;uk='Помилка при створенні опису вебсервіса (%1).
            |Помилка читання WSDL-опису вебсервіса перевірки легальності завантаження оновлення: %2.'"),
			АдресWSDL, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстИсключения;
	КонецПопытки;
	...
	Если УзлыСхемы.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |;uk='Помилка при створенні опису вебсервіса (%1).
            |Помилка читання WSDL-опису вебсервіса перевірки легальності завантаження оновлення:
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |%2'
            |;uk='Помилка при створенні опису вебсервіса (%1).
            |Помилка при створенні схеми даних з WSDL-опису вебсервіса перевірки легальності завантаження оновлення:
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
            ...
            |;uk='Помилка при створенні опису вебсервіса (%1).
            |Помилка при створенні фабрики XDTO з WSDL-опису вебсервіса перевірки легальності завантаження оновлення: %2'"),
			АдресWSDL, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстИсключения;
	...
	Если НЕ ЗначениеЗаполнено(ИмяПорта) Тогда
		...
		ТекстИсключения = СтрЗаменить(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
                                            ...
                                            |;uk='Помилка при створенні опису вебсервісу (%1).
                                            |Помилка читання WSDL-опису вебсервісу інтернет-підтримки користувачів:
	...
	Если НЕ ЗначениеЗаполнено(АдресПорта) Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка при создании описания веб-сервиса (%1).
                ...
                |;uk='Помилка при створенні опису вебсервіса (%1).
                |Помилка читання WSDL-опису вебсервіса інтернет-підтримки користувачів:
