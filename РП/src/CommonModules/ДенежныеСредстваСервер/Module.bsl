...
Процедура ЗаполнитьДокументПоЗаявкеНаРасходованиеДенежныхСредств(
	...
	ИначеЕсли ФормаОплаты = Перечисления.ФормыОплаты.ПлатежнаяКарта
		...
		Текст = НСтр("ru='Выбранная операция не соответствует операции возврата оплаты клиенту, для которой предусмотрен эквайринговый платеж';uk='Вибрана операція не відповідає операції повернення оплати клієнту, для якої передбачено еквайринговий платіж'");
		ВызватьИсключение Текст;
	КонецЕсли;
	...
	ИначеЕсли Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыЧерезКассу
		...
		Если ТабВедомости.Количество() > 0 Тогда
			...
			Если ТипЗнч(ТабВедомости[0].Ведомость) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыРаздатчиком") Тогда
				
				ДанныеЗаполнения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыРаздатчиком;
				ДанныеЗаполнения.Вставить("ПодотчетноеЛицо", ТабВедомости[0].Ведомость.Раздатчик);
...
Процедура ДанныеОбОплатеВедомостей(МенеджерВременныхТаблиц, Параметры) Экспорт
	...
	|			ИЛИ ДанныеДокумента.Ссылка В (&МассивВедомостей))
	|	И (&БезОтбораПоКассе
	|			ИЛИ ДанныеДокумента.Касса В (&Касса)
	|			ИЛИ ДанныеДокумента.Касса = ЗНАЧЕНИЕ(Справочник.Кассы.ПустаяСсылка))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			ИЛИ ДанныеДокумента.Раздатчик В (&Раздатчик))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ДанныеВедомостейНаВыплатуЗарплаты(МассивВедомостей) Экспорт
	...
	Если РеквизитыШапки.КоличествоЗарплатныхПроектов > 1 Тогда
		Текст = НСтр("ru='У выделенных ведомостей отличается поле ""Зарплатный проект"". Ввод одного документа на основании выделенных ведомостей невозможен.';uk='У виділених відомостей відрізняється поле ""Зарплатний проєкт"". Введення одного документа на підставі виділених відомостей неможливе.'");
