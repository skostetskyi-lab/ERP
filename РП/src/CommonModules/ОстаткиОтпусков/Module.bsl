		...
		Для Каждого СтрокаВидаОтпуска Из ТаблицаВидовОтпускаПоСотруднику Цикл
			...
			Для Каждого РабочийГод Из РабочиеГодаПоВидуОтпуска Цикл
				...
				Иначе  
					...
					// В таблиці прав шукаємо актуальні права на робочий рік : 
					ТаблицаПраваПоВидуОтпускаЗаРабочийГод =  Новый ТаблицаЗначений;
					ТаблицаПраваПоВидуОтпускаЗаРабочийГод.Колонки.Добавить("ДатаДействия", Новый ОписаниеТипов("Дата"));
					ТаблицаПраваПоВидуОтпускаЗаРабочийГод.Колонки.Добавить("КоличествоДней", Новый ОписаниеТипов("Число"));
					Для Каждого Право Из ПраваНаЕжегодныйОтпускПоВидуОтпуска Цикл  
						НоваяСтрока = ТаблицаПраваПоВидуОтпускаЗаРабочийГод.Добавить();
						НоваяСтрока.ДатаДействия = Право.ДатаДействия;
						НоваяСтрока.КоличествоДней = Право.КоличествоДнейОтпускаВГод;
					КонецЦикла;
					
					Запрос.УстановитьПараметр("ТаблицаПраваПоВидуОтпускаЗаРабочийГод", ТаблицаПраваПоВидуОтпускаЗаРабочийГод);
					Запрос.УстановитьПараметр("ДатаНачала", РабочийГод.ДатаНачала);
					Запрос.УстановитьПараметр("ДатаОкончания", РабочийГод.ДатаОкончания); 
					
					Запрос.Текст = "ВЫБРАТЬ
						|	ТаблицаПраваПоВидуОтпускаЗаРабочийГод.ДатаДействия КАК ДатаДействия,
						|	ТаблицаПраваПоВидуОтпускаЗаРабочийГод.КоличествоДней КАК КоличествоДней
						|ПОМЕСТИТЬ ВТТаблицаПраваПоВидуОтпускаЗаРабочийГод
						|ИЗ
						|	&ТаблицаПраваПоВидуОтпускаЗаРабочийГод КАК ТаблицаПраваПоВидуОтпускаЗаРабочийГод
						|ГДЕ
						|	ТаблицаПраваПоВидуОтпускаЗаРабочийГод.ДатаДействия МЕЖДУ &ДатаНачала И &ДатаОкончания
						|;
						|
						|////////////////////////////////////////////////////////////////////////////////
						|ВЫБРАТЬ
						|	МАКСИМУМ(ТаблицаПраваПоВидуОтпускаЗаРабочийГод.ДатаДействия) КАК ДатаДействия,
						|	ТаблицаПраваПоВидуОтпускаЗаРабочийГод.КоличествоДней КАК КоличествоДней
						|ИЗ
						|	ВТТаблицаПраваПоВидуОтпускаЗаРабочийГод КАК ТаблицаПраваПоВидуОтпускаЗаРабочийГод
						|
						|СГРУППИРОВАТЬ ПО
						|	ТаблицаПраваПоВидуОтпускаЗаРабочийГод.КоличествоДней";   
					Выборка = Запрос.Выполнить().Выбрать();
					Если Выборка.Следующий() Тогда
						// на кожен робочий рік береться остання введена в цьому році (або раніше) кількість днів щорічної відпустки
						КоличествоДнейОтпускаВГод = Выборка.КоличествоДней;	
					КонецЕсли;
					
					Запрос.Текст = "
					|УНИЧТОЖИТЬ ВТТаблицаПраваПоВидуОтпускаЗаРабочийГод";
					Запрос.Выполнить();
					...
					Иначе // Отпуск попадает на текущий рабочий год.
						...
						Если Не ЗначениеЗаполнено(СтрокаФактическогоОтпуска.РабочийГодС) Тогда
							...
							Если Не РассчитыватьОстатокОтпускаПоКалендарнымДням Тогда
								...
								КонецЕсли; 
							Иначе
								Если (РабочийГод.ОстатокНаКонецТекущегоГода + КоличествоДнейОтпускаВГод - РабочийГод.КоличествоДнейЗаПолныйГод) <= 0 
									И НЕ (РабочийГод.ОстатокНаКонецТекущегоГода = 0 И КоличествоДнейОтпускаВГод = 0 И КоличествоДнейЗаГод = 0     
						...
						Если СтрокаФактическогоОтпуска.ОсталосьСписать <> 0 Тогда
							...
							Иначе
									...
									// КоличествоДнейОтпускаВГод - належить кількість днів щорічної відпустки за повний робочий рік
									// РабочийГод.КоличествоДнейЗаПолныйГо - скільки зароблено за цей робочий рік (РабочийГод) на дату відпустки    
									// СтрокаФактическогоОтпуска.ИспользованоДнейАвансом - кількість днів, взятих "наперед" 
									Если КоличествоДнейОтпускаВГод <> 0 И ((КоличествоДнейОтпускаВГод - РабочийГод.КоличествоДнейЗаПолныйГод) < СтрокаФактическогоОтпуска.ИспользованоДнейАвансом 
										ИЛИ (СтрокаФактическогоОтпуска.РабочийГодПо < СтрокаФактическогоОтпуска.РабочийГодС)) Тогда
										СтрокаФактическогоОтпуска.РабочийГодПо			= ДобавитьМесяц(СтрокаФактическогоОтпуска.РабочийГодПо, 12);      
									КонецЕсли;
							КонецЕсли;	
					...
					Пока Выборка.СледующийПоЗначениюПоля("ВидЕжегодногоОтпуска") Цикл    
						...
						Пока Выборка.СледующийПоЗначениюПоля("РабочийГодДатаНачала") Цикл 
							...
							Пока Выборка.Следующий() Цикл    
								...
								Локализация.ПолучитьЛокализованноеПредставление(Выборка.Состояние, Локализация.КодЯзыкаИнтерфейса()),
...
Процедура ЗаполнитьОтпускВоенногоПоложения() Экспорт
	...
	ОбъектЗаполнения.Наименование = НСтр("ru='Отпуск неоплачиваемый (на время военного положения)';uk='Неоплачувана відпустка (на час воєнного стану)'",Локализация.КодЯзыкаИнформационнойБазы());
	ОбъектЗаполнения.НаименованиеПолное = НСтр("ru='Отпуск неоплачиваемый (на время военного положения)';uk='Неоплачувана відпустка (на час воєнного стану)'",Локализация.КодЯзыкаИнформационнойБазы());
