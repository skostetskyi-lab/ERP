...
//		КонецПериода
//
Функция СреднесписочнаяЧисленностьРаботающих(Организация, НачалоПериода, КонецПериода, ИсключатьПериодыПоПравилам1ПВ = Истина,  ИсключатьТяжелыеУсловияТруда = Ложь) Экспорт 
	...
	СтатистикаПерсонала.СреднесписочнаяЧисленностьРаботающих(Организация, НачалоПериода, КонецПериода, ДанныеЧисленности, ИсключатьПериодыПоПравилам1ПВ, ИсключатьТяжелыеУсловияТруда);
	
	Возврат ДанныеЧисленности;
	
КонецФункции  

// Функція розраховує середню за списком чисельність в еквіваленті повної занятості 
//
// Параметри:
//		Организация
//		НачалоПериода
//		КонецПериода
//
Функция СреднесписочнаяЧисленностьРаботающихЭПЗ(Организация, НачалоПериода, КонецПериода, ИсключатьТяжелыеУсловияТруда = Ложь) Экспорт 
	
	УстановитьПривилегированныйРежим(Истина);
	
	ДанныеЧисленности = Новый Структура("ЧисленностьРаботниковЭПЗВнешнихСовместителей, ЧисленностьРаботниковЭПЗОсновыныхСотрудников, ЧисленностьРаботниковЭПЗВнутреннихСовместителей");
	СтатистикаПерсонала.СреднесписочнаяЧисленностьРаботающихЭПЗ(Организация, НачалоПериода, КонецПериода, ДанныеЧисленности, ИсключатьТяжелыеУсловияТруда);
...
Функция НеобходимыСведенияОВоинскомУчетеФизическихЛиц(Знач ИмяПоля)
	...
	Возврат ИмяПоля = ВРег("ВоинскийУчетПериодРегистрации")
		...
		Или ИмяПоля = ВРег("ВоинскийУчетЗабронированСтороннейОрганизацией")
		Или ИмяПоля = ВРег("ВоинскийУчетЗабронированСтороннейОрганизациейСрок")
...
Функция ПутьКДаннымСведенийОВоинскомУчетеФизическихЛиц(Знач ИмяПоля)
	...
	ИначеЕсли ИмяПоля = ВРег("ВоинскийУчетЗабронированСтороннейОрганизацией") Тогда
		ПутьКДанным = "	ВоинскийУчет.ЗабронированСтороннейОрганизацией";
	ИначеЕсли ИмяПоля = ВРег("ВоинскийУчетЗабронированСтороннейОрганизациейСрок") Тогда
		ПутьКДанным = "	ВоинскийУчет.ЗабронированСтороннейОрганизациейСрок";
    КонецЕсли;
...
Функция НеобходимыСведенияОБронированииСотрудников(Знач ИмяПоля)
	...
	Возврат ВРег(ИмяПоля) = ВРег("ВоинскийУчетЗабронированОрганизациейПериодРегистрации")
		...
		Или ВРег(ИмяПоля) = ВРег("ВоинскийУчетЗабронированОрганизациейСрокБронирования")
...
Функция ДобавитьКритерийПоискаПоСведениямОБронированииГражданОрганизациями(КритерииПоиска, УсловиеПоиска)
	...
	Если НеобходимыСведенияОБронированииСотрудников(УсловиеПоиска.ЛевоеЗначение) Тогда
		...
		ИначеЕсли ИмяПоля = ВРег("ВоинскийУчетЗабронированОрганизациейСрокБронирования") Тогда
			УсловиеПоиска.ЛевоеЗначение = "СрокБронирования";
		КонецЕсли;
...
Функция ПутьКДаннымСведенийОБронированииСотрудников(Знач ИмяПоля)
	...
	ИначеЕсли ИмяПоля = ВРег("ВоинскийУчетЗабронированОрганизациейСрокБронирования") Тогда
		ПутьКДанным = "	БронированиеСотрудников.СрокБронирования";
	КонецЕсли;
...
//		ДатаСобытия					- Дата
//		ИменаТабличныхЧастейНачисления - Строка
...
//
Процедура ПроверкаСпискаНачисленийКадровогоДокумента(ДокументОбъект, ДатаСобытия, ИменаТабличныхЧастейНачисления, ИмяТабличнойЧастиПоказатели, Отказ, СодержитПолеДействие = Ложь, ИмяРеквизитаРабочееМесто = "", ИменаРеквизитовНачисление = "Начисление", ИмяТабличнойЧастиСотрудники = "") Экспорт	
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ПустаяСтрока(ИмяРеквизитаРабочееМесто) Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Заданы одинаковые начисления ""%1"" в строках %2 и %3';uk='Задано однакові нарахування ""%1"" у рядках %2 і %3'"), Выборка.Начисление, Выборка.НомерСтроки, Выборка.НомерСтрокиВторой);
				...
				Иначе
					...
					Если Не ПустаяСтрока(ИмяТабличнойЧастиСотрудники) 
						...
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='[%1] заданы одинаковые начисления ""%2""';uk='[%1] задано однакові нарахування ""%2""'"),
							РабочееМестоПредставление, Выборка.Начисление);
						
					Иначе
						
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='[%1] заданы одинаковые начисления ""%2"" в строках %3 и %4';uk='[%1] задано однакові нарахування ""%2"" у рядках %3 і %4'"),
...
//~	Обработчик.Процедура = "КадровыйУчетРасширенный.ЗаполнитьВидыДоговоров";
//-- ЕРП ЗИК      
...
Процедура ЗаполнитьВидыДоговоров() Экспорт
	...
	|	И ПриемНаРаботу.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровССотрудниками.ПустаяСсылка)
	|; 
	|ВЫБРАТЬ
	|	ПриемНаРаботуСписком.Ссылка КАК Регистратор
	|ИЗ
	|	Документ.ПриемНаРаботуСписком КАК ПриемНаРаботуСписком
	|ГДЕ
	|	ПриемНаРаботуСписком.ПометкаУдаления = ЛОЖЬ
	|	И ПриемНаРаботуСписком.Проведен = Истина
	|	И ПриемНаРаботуСписком.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровССотрудниками.ПустаяСсылка)
	|; 
	|ВЫБРАТЬ
	...
	|	И КадровыйПеревод.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровССотрудниками.ПустаяСсылка)
	|; 
	|ВЫБРАТЬ
	|	КадровыйПереводСписком.Ссылка КАК Регистратор
	|ИЗ
	|	Документ.КадровыйПереводСписком КАК КадровыйПереводСписком
	|ГДЕ
	|	КадровыйПереводСписком.ПометкаУдаления = ЛОЖЬ
	|	И КадровыйПереводСписком.Проведен = Истина
	|	И КадровыйПереводСписком.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровССотрудниками.ПустаяСсылка)
	|; 
	|ВЫБРАТЬ
	|	НачальнаяШтатнаяРасстановка.Ссылка КАК Регистратор
	|ИЗ
	|	Документ.НачальнаяШтатнаяРасстановка КАК НачальнаяШтатнаяРасстановка
	|ГДЕ
	|	НачальнаяШтатнаяРасстановка.ПометкаУдаления = ЛОЖЬ
	|	И НачальнаяШтатнаяРасстановка.Проведен = Истина
	|	И НачальнаяШтатнаяРасстановка.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровССотрудниками.ПустаяСсылка)
	|; 
	|ВЫБРАТЬ
	...
	|; 
	|ВЫБРАТЬ
	|	УвольнениеСписком.Ссылка КАК Регистратор
	|ИЗ
	|	Документ.УвольнениеСписком КАК УвольнениеСписком
	|ГДЕ
	|	УвольнениеСписком.ПометкаУдаления = ЛОЖЬ
	|	И УвольнениеСписком.Проведен = Истина
    |";
	...
	Для Инд = 0 По 6 Цикл
		Если НЕ РезультатЗапроса[Инд].Пустой() Тогда
			...
			ЗаполнитьИЗаписатьВидДоговора(Выборка);
		
		КонецЕсли; 
	КонецЦикла; 
	
	ТрудовойДоговор = Перечисления.ВидыДоговоровССотрудниками.ТрудовойДоговор;    

	НаборЗаписей = РегистрыСведений.ТекущиеКадровыеДанныеСотрудников.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
	
	ВидДоговораЗаполнялся = Ложь;	
    Для каждого ЗаписьНабора Из НаборЗаписей Цикл 
		Если Не ЗначениеЗаполнено(ЗаписьНабора.ТекущийВидДоговора) И ЗначениеЗаполнено(ЗаписьНабора.ТекущийВидЗанятости) Тогда
			ЗаписьНабора.ТекущийВидДоговора = ТрудовойДоговор;
			ВидДоговораЗаполнялся = Истина
		КонецЕсли	
	КонецЦикла;
		
	Если ВидДоговораЗаполнялся Тогда
...
Процедура ЗаполнитьИЗаписатьВидДоговора(ДанныеДокумента) Экспорт
	
	ТрудовойДоговор = Перечисления.ВидыДоговоровССотрудниками.ТрудовойДоговор;    
	
	Пока ДанныеДокумента.Следующий() Цикл
		
		ДокументОбъект = ДанныеДокумента.Регистратор.ПолучитьОбъект(); 
		ВидДоговораЗаполнялся = Ложь;	
		Если НЕ ЗначениеЗаполнено(ДокументОбъект.ВидДоговора) Тогда 
			ДокументОбъект.ВидДоговора = ТрудовойДоговор;
			ВидДоговораЗаполнялся = Истина
		КонецЕсли;
		Если ВидДоговораЗаполнялся Тогда
		...
		НаборЗаписей = РегистрыСведений.КадроваяИсторияСотрудников.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Регистратор.Установить(ДанныеДокумента.Регистратор);
		
		НаборЗаписей.Прочитать();
		
		ВидДоговораЗаполнялся = Ложь;	
        Для каждого ЗаписьНабора Из НаборЗаписей Цикл 
			Если Не ЗначениеЗаполнено(ЗаписьНабора.ВидДоговора) Тогда
				ЗаписьНабора.ВидДоговора = ТрудовойДоговор;
				ВидДоговораЗаполнялся = Истина
			КонецЕсли	
		КонецЦикла;
