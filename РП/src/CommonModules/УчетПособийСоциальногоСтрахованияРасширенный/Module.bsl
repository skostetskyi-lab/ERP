...
Функция ДанныеДляРасчетаСреднегоЗаработкаФСССотрудников(ИсходныеДанные, ДополнительныеПараметры)
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВТДанныеВремениБезУчетаВыходных
		|ИЗ
		|	ВТИсходныеДанные КАК ИсходныеДанные
		...
		|	ДанныеВремени.ДнейБолезниУходаЗаДетьми КАК ДнейБолезниУходаЗаДетьми, 
		|	ДанныеВремени.Источник КАК Источник 
		|ИЗ ВТДанныеВремениБезУчетаВыходных КАК ДанныеВремени
		|УПОРЯДОЧИТЬ ПО
	...
	// Згідно Постанови № 837 від 14.07.25 р.:
	Если ДополнительныеПараметры.ДатаНачалаСобытия >= Дата('20250718') И ДанныеОВремени.Итог("ДнейБолезниУходаЗаДетьми") <> 0 Тогда
		Запрос.Текст = "ВЫБРАТЬ
		|	Начисления.Ссылка КАК ВидРасчета
		|ИЗ
		|	ПланВидовРасчета.Начисления КАК Начисления
		...
		СписокВидовРасчетаРабота = Новый Массив;
		СписокВидовРасчетаРабота = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ВидРасчета");
		Запрос.УстановитьПараметр("СписокВидовРасчетаРабота", СписокВидовРасчетаРабота);
		...
		|	ФПД.ВидРасчета,
		|	СУММА(ФПД.ОтработаноДней) КАК ОтработаноДней,
		|	СУММА(ФПД.ОтработаноЧасов) КАК ОтработаноЧасов,
		|	ФПД.ГрафикРаботы,
		|	ФПД.Регистратор
		|ПОМЕСТИТЬ ВТФПД
		|ИЗ
		...
		|					ИЗ
		|						ВТИсходныеДанные КАК ИсходныеДанные)
		|				И ВидРасчета В (&СписокВидовРасчетаРабота)) КАК ФПД
		|ГДЕ
		|	ФПД.ОтработаноДней = 0
		|	И ФПД.ОтработаноЧасов = 0 
		|
		|СГРУППИРОВАТЬ ПО
		|	ФПД.Сотрудник,
		|	ФПД.ГоловнаяОрганизация,
		...
		|	НАЧАЛОПЕРИОДА(ФПД.ПериодДействияНачало, МЕСЯЦ) КАК Период,
		|	СУММА(ДЕНЬ(ФПД.ПериодДействияКонец) - ДЕНЬ(ФПД.ПериодДействияНачало) + 1) КАК КоличествоНеотработанных
		|ПОМЕСТИТЬ ВТПериодыРаботы
		|ИЗ ВТФПД КАК ФПД 
		|ГДЕ ФПД.ВидРасчета В (&СписокВидовРасчетаРабота)
		|СГРУППИРОВАТЬ ПО
		|	ФПД.Сотрудник,
		|	ФПД.ГоловнаяОрганизация,
		|   НАЧАЛОПЕРИОДА(ФПД.ПериодДействияНачало, МЕСЯЦ),
		...
		|	ДанныеВремени.ГоловнаяОрганизация,
		|	ДанныеВремени.Период, 
		|	ВЫБОР КОГДА (ЕСТЬNULL(ПериодыРаботы.КоличествоНеотработанных, 0) + ДанныеВремени.ДнейБолезниУходаЗаДетьми) = ДЕНЬ(КОНЕЦПЕРИОДА(ДанныеВремени.Период, МЕСЯЦ))
		|		И ПериодыРаботы.ОтработаноДней = 0 И ПериодыРаботы.ОтработаноЧасов = 0 
		|         ТОГДА 0
		|   ИНАЧЕ
		...
		|ПО ДанныеВремени.Сотрудник = ПериодыРаботы.Сотрудник
		|   И ДанныеВремени.ГоловнаяОрганизация = ПериодыРаботы.ГоловнаяОрганизация 
		|	И ДанныеВремени.Период = ПериодыРаботы.Период
		|УПОРЯДОЧИТЬ ПО
		|	ДанныеВремени.ФизическоеЛицо,
		|	ДанныеВремени.Сотрудник,
		|	ДанныеВремени.Период
