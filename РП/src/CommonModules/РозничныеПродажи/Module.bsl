...
Функция ЗаполнитьОтчетОРозничныхПродажах(ОтчетОРозничныхПродажахОбъект, КассоваяСмена, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ВЗ.СуммаОкругления) КАК СуммаОкругления
		|ИЗ
		|	(ВЫБРАТЬ
		|		ЧекККМ.СуммаОкругления КАК СуммаОкругления
		|	ИЗ
		|		Документ.ЧекККМ КАК ЧекККМ
		|	ГДЕ
		|		ЧекККМ.Ссылка В
		...
		ВыборкаОкругление = МассивРезультатов[8].Выбрать();
		ВыборкаОкругление.Следующий();
		...
		Пока ВыборкаПоАналитике.Следующий() Цикл
			...
			ОтчетОРозничныхПродажахОбъект.СуммаОкругления = ВыборкаОкругление.СуммаОкругления;
			ОтчетОРозничныхПродажахОбъект.СуммаДокумента  = ЦенообразованиеКлиентСервер.ПолучитьСуммуДокумента(ОтчетОРозничныхПродажахОбъект.Товары, ОтчетОРозничныхПродажахОбъект.ЦенаВключаетНДС)
				+ ОтчетОРозничныхПродажахОбъект.СуммаОкругления;
