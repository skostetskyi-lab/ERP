...
Функция ЗакрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаОплат, ВыходныеПараметры) Экспорт
	...
	Иначе
		Если ПараметрыПодключения.ФискальныйЧек Тогда 
			ТекстыОплаты = Новый Массив;
			СуммаОплат = 0;
			СуммаОкругления = 0;
			СуммаДокумента = ПараметрыПодключения.ИтогЧека;
			
			Для каждого СтрокаОплаты Из ТаблицаОплат Цикл
				
				СуммаСтроки = СтрокаОплаты[1].Значение;
				СуммаОплат = СуммаОплат + СуммаСтроки;
				
				Если СтрокаОплаты[0].Значение = 0 Тогда
					ТекстыОплаты.Вставить(0, НСтр("ru='НАЛИЧНЫЕ';uk='ГОТІВКА'") + ": " + Формат(СуммаСтроки, "ЧЦ=15; ЧДЦ=2; ЧН=0; ЧГ=0"));
					Если СтрокаОплаты.Количество() > 4 Тогда
						Если СтрокаОплаты[4].Значение.Свойство("СуммаОкругления") Тогда
							СуммаОкругления = СтрокаОплаты[4].Значение.СуммаОкругления;
						КонецЕсли;
						Если СтрокаОплаты[4].Значение.Свойство("СуммаДокумента") Тогда
							СуммаДокумента = СтрокаОплаты[4].Значение.СуммаДокумента;
						КонецЕсли;
					КонецЕсли;
				Иначе
					ТекстыОплаты.Добавить(НСтр("ru='БЕЗНАЛИЧНАЯ';uk='БЕЗГОТІВКОВА'") + ": " + Формат(СуммаСтроки, "ЧЦ=15; ЧДЦ=2; ЧН=0; ЧГ=0"));
					ТекстыОплаты.Добавить(СтрокаОплаты[2].Значение);
				КонецЕсли;
				
			КонецЦикла; 

			ТекстыОплаты.Добавить(Символы.ПС + НСтр("ru='СУММА';uk='СУМА'") + ": " + Формат(ПараметрыПодключения.ИтогЧека, "ЧЦ=15; ЧДЦ=2; ЧН=0; ЧГ=0"));

			Если ЗначениеЗаполнено(СуммаОкругления) Тогда
				ТекстыОплаты.Добавить(НСтр("ru='Округление';uk= 'Заокруглення'") + ": " + Формат(СуммаОкругления, "ЧЦ=15; ЧДЦ=2; ЧН=0; ЧГ=0"));
				ТекстыОплаты.Добавить(НСтр("ru='К оплате';uk= 'До сплати'") + ": " + Формат(СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧН=0; ЧГ=0"));
			КонецЕсли;

			Сдача = СуммаОплат - СуммаДокумента;
			Если Сдача >  0 Тогда
				ТекстыОплаты.Добавить(НСтр("ru='Сдача';uk='Здача'") + ": "      + Формат(Сдача, "ЧЦ=15; ЧДЦ=2; ЧН=0; ЧГ=0"));
			КонецЕсли;

			СтрокаЛога = СтрокаЛога + СтрСоединить(ТекстыОплаты, Символы.ПС) + Символы.ПС; 
		КонецЕсли;
