...
Функция СведенияОбАдресеВВидеСтруктуры(Адрес, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Формат9Запятых Тогда
		...
		ИначеЕсли XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()) Тогда
			АдресНациональный = Состав;
		Иначе
			Возврат Результат;
		КонецЕсли;
		
		Если ТипЗнч(АдресНациональный) = Тип("Строка") Тогда
			Если СтрЧислоВхождений(АдресНациональный, ",") = 9 Тогда
				ЭлементыАдреса = СтруктураАдресаПоФормату9Запятых(АдресНациональный);
				Формат9Запятых = Истина;
			Иначе                                    
			...
			КонецЕсли;
		ИначеЕсли ТипЗнч(АдресНациональный) = Тип("Неопределено") Тогда
		...
		Иначе
			ЭлементыАдреса.Вставить("Страна", НаименованиеСтраны);
			ЭлементыАдреса.Вставить("Индекс", ПочтовыйИндексАдреса(АдресНациональный));
			ЭлементыАдреса.Вставить("Регион", АдресНациональный["Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()]);
			ЭлементыАдреса.Вставить("Район", РайонАдреса(АдресНациональный));
			ЭлементыАдреса.Вставить("Город", АдресНациональный.Город);
			ЭлементыАдреса.Вставить("НаселенныйПункт", АдресНациональный.НаселПункт);
			ЭлементыАдреса.Вставить("Улица", АдресНациональный.Улица);
			ЭлементыАдреса.Вставить("Округ", АдресНациональный.Округ);
			ЭлементыАдреса.Вставить("ВнутригРайон", АдресНациональный.ВнутригРайон);
			ЭлементыАдреса.Вставить("Дом","");
			ЭлементыАдреса.Вставить("Корпус", "");
			ЭлементыАдреса.Вставить("Квартира", "");
			ЭлементыАдреса.Вставить("ДополнительнаяТерритория", НайтиДополнительныйЭлементАдреса(АдресНациональный));
			ЭлементыАдреса.Вставить("ЭлементДополнительнойТерритории", 
				ДополнительныйЭлементАдреса(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(91)));
		КонецЕсли;
	КонецЕсли;
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Текст) Тогда
		...
		Если ТекстОшибки = Неопределено Тогда
			...
			ИначеЕсли ОжидаемыйТип = ПеречислениеВебСтраница И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ВебСайт") Тогда
				ТекстОшибки = НСтр("ru='Ошибка десериализации контактной информации, ожидается веб-страница';uk='Помилка десеріалізації контактної інформації, очікується вебстрінка'");
...
Процедура ПоместитьАдресВXDTO(АдресНациональный, АдресПоУровням)
	...
	Для каждого ЭлементАдреса Из АдресПоУровням Цикл
		...
		Если ЭлементАдреса.Уровень = 1 Тогда
			Путь = "Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный();
		...
		ИначеЕсли ЭлементАдреса.Уровень = 90 Тогда
			ДобавитьДополнительныеЭлементыАдреса(АдресНациональный, ЭлементАдреса.Значение, 90);
		ИначеЕсли ЭлементАдреса.Уровень = 91 Тогда
			ДобавитьДополнительныеЭлементыАдреса(АдресНациональный, ЭлементАдреса.Значение, 91);
		ИначеЕсли ЭлементАдреса.Уровень = -3 Тогда
			Продолжить;
		ИначеЕсли ЭлементАдреса.Уровень = -1 Тогда
			ПочтовыйИндексАдреса(АдресНациональный, ЭлементАдреса.Наименование);
		Иначе
		...
		Если ЭлементАдреса.Уровень > 0 И ЭлементАдреса.Уровень < 90 Тогда
			УстановитьСвойствоПоXPath(АдресНациональный, Путь, ЭлементАдреса.Значение);
		КонецЕсли;
		
	КонецЦикла;
	
	ЗданияИПомещения = Новый Структура("Здания, Помещения", ТаблицаЗданийПомещений, ТаблицаЗданийПомещений);
	ЗданияИПомещенияАдреса(АдресНациональный, ЗданияИПомещения);
...
Функция АдресВведенВСвободнойФорме(XDTOИнформация, НовоеЗначение = Неопределено) Экспорт
	...
	АдресНациональный = ОбъектXDTO.Состав.Состав;
	Если ТипЗнч(НовоеЗначение) <> Тип("Булево") Тогда
		// Читаем
		Возврат Не ПустаяСтрока(АдресНациональный.Адрес_по_документу);
	КонецЕсли;
		
	// Устанавливаем
	Если НовоеЗначение Тогда
		АдресНациональный.Адрес_по_документу = ОбъектXDTO.Представление;
	Иначе
		АдресНациональный.Сбросить("Адрес_по_документу");
	КонецЕсли;
...
//  Возвращает флаг того, что переданный адрес - национальный.
//
//  Параметры:
...
Функция НациональныйАдрес(ОбъектИнформации) Экспорт
	...
	Если ТипЗнч(ОбъектИнформации) = ТипXDTO Тогда
		...
		Если ТипЗнч(Адрес) = ТипXDTO И Адрес.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
			Адрес = Адрес.Состав;
		КонецЕсли;
		Если ТипЗнч(Адрес) = ТипXDTO И Адрес.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()) Тогда
			Результат = Адрес;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗначениеДополнительныхЭлементов(Знач XDTOАдрес)
	...
	ДополнительныйЭлементАдреса = НайтиДополнительныйЭлементАдреса(АдресНациональный);

	Результат.ДополнительныйЭлемент = ДополнительныйЭлементАдреса;    
	Результат.ПодчиненныйЭлемент = ДополнительныйЭлементАдреса(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(91));
...
Функция ПочтовыйИндексАдреса(XDTOАдрес, НовоеЗначение = Неопределено) Экспорт
	...
	Если НовоеЗначение = Неопределено Тогда
		// Чтение
		Результат = АдресНациональный.Получить( УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса() );
		Если Результат <> Неопределено Тогда
	...
	ЗаписьИндекса = АдресНациональный.Получить(УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса());
...
//                   
Функция НайтиДополнительныйЭлементАдреса(АдресНациональный) Экспорт
	
	ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90, "СНТ"));
	Если ДополнительныйЭлементАдреса = Неопределено Тогда
		ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90, "ГСК"));
		Если ДополнительныйЭлементАдреса = Неопределено Тогда
			ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90, "ТЕР"));
			Если ДополнительныйЭлементАдреса = Неопределено Тогда
				ДополнительныйЭлементАдреса = ДополнительныйЭлементАдреса(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(90));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьДополнительныеЭлементыАдреса(XDTOАдрес, НовоеЗначение, Уровень) Экспорт
	...
	ЗначениеПоля = АдресНациональный.Получить(XPathЭлемента);
	Если ЗначениеПоля = Неопределено Тогда
		ЗначениеПоля = АдресНациональный.ДопАдрЭл.Добавить(ФабрикаXDTO.Создать(XDTOАдрес.ДопАдрЭл.ВладеющееСвойство.Тип));
...
Функция ОпределитьПочтовыйИндексИОКТМОАдреса(XDTOАдрес, Идентификатор = Неопределено) Экспорт
	...
	Если XDTOАдрес.Тип() = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда 
		XDTOАдресНациональный = XDTOАдрес.Состав;
	Иначе 
		XDTOАдресНациональный = XDTOАдрес;
	КонецЕсли;
	
	Если XDTOАдресНациональный = Неопределено Или XDTOАдресНациональный.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()) Тогда
...
Функция РайонАдреса(XDTOАдрес, НовоеЗначение = Неопределено)
	...
	Если НовоеЗначение = Неопределено Тогда
		...
		Если XDTOТип = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()) Тогда
		...
		Если ТипЗнч(АдресНациональный) = Тип("ОбъектXDTO") Тогда
			Возврат ПолучитьXDTOРеквизитОбъекта(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathРайона());
		КонецЕсли;
...
Функция ПредставлениеНаселенногоПункта(ОбъектАдреса) Экспорт
	...
	АдресНациональный = НациональныйАдрес(ОбъектАдреса);
	Если АдресНациональный = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	...
	Если АдресНациональный.СвРайМО = Неопределено Тогда
		Район = "";        
	ИначеЕсли АдресНациональный.СвРайМО.Район <> Неопределено Тогда
		Район = АдресНациональный.СвРайМО.Район;
	ИначеЕсли АдресНациональный.СвРайМО.СвМО <> Неопределено Тогда
		Районы = Новый Массив(АдресНациональный.СвРайМО.СвМО.МунОбр2, АдресНациональный.СвРайМО.СвМО.МунОбр1);
	...
	Адрес = Новый Массив;         
	Адрес.Добавить(АдресНациональный.НаселПункт);
	Адрес.Добавить(АдресНациональный.ВнутригРайон);
	Адрес.Добавить(АдресНациональный.Город);
	Адрес.Добавить(Район);
	Адрес.Добавить(АдресНациональный.Округ);
	Адрес.Добавить(АдресНациональный["Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()]);
...
Функция ПредставлениеУлицы(ОбъектАдреса) Экспорт
	...
	Адрес = Новый Массив; 
	Адрес.Добавить(АдресНациональный.Улица);
	...
	Адрес.Добавить(ДополнительныйЭлементАдреса(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathДополнительногоОбъектаАдресации(91)));
...
Функция ПредставлениеАдреса(Знач XDTOАдрес, Знач ВидИнформации) Экспорт
	...
	Если ЭтоНациональныйАдрес(АдресНациональный) Тогда
		
		// Значимые части
		Адрес = Новый Массив;
		Адрес.Добавить(ДополнительныйЭлементАдреса(АдресНациональный, УправлениеКонтактнойИнформациейКлиентСервер.XPathПочтовогоИндекса()));
		Адрес.Добавить(АдресНациональный["Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()]);
		Если ВидИнформации.ФорматАдреса = "ФИАС" Тогда
			Адрес.Добавить(АдресНациональный.Округ);
		КонецЕсли;
		Адрес.Добавить(РайонАдреса(АдресНациональный));
		Адрес.Добавить(АдресНациональный.Город);
		Если ВидИнформации.ФорматАдреса = "ФИАС" Тогда
			Адрес.Добавить(АдресНациональный.ВнутригРайон);
		КонецЕсли;
		...
		Данные = ЗданияИПомещенияАдреса(АдресНациональный);
	...
	Иначе
		...
		Представление = СокрЛП(АдресНациональный);
...
Функция ОшибкиЗаполненияАдресаXDTO(XDTOАдрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Иначе
		...
		АдресНациональный = ?(Адрес = Неопределено, Неопределено, Адрес.Состав);
	КонецЕсли;
	...
	ПространствоИмен = УправлениеКонтактнойИнформациейКлиентСервер.ПространствоИмен();
	Если ТипЗнч(АдресНациональный) <> Тип("ОбъектXDTO") Или АдресНациональный.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()) Тогда
	...
	Если Не XDTOКонтактнаяИнформацияЗаполнена(АдресНациональный) Тогда
		// Адрес пуст
	...
	ВсеОшибки = ОбщиеГруппыОшибокЗаполненияАдреса(АдресНациональный, ФлагиПроверки);
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Если ТипЗнч(ДанныеАдреса) = Тип("ОбъектXDTO") Тогда
		АдресНациональный = ДанныеАдреса;
	...
	Иначе
		...
		Если Не ЭтоНациональныйАдрес(Адрес) Тогда
			Возврат Результат;
		КонецЕсли;
		АдресНациональный = Адрес.Состав;
		
		// С) совпадение представления и набора данных.
		Представление = ПредставлениеАдреса(АдресНациональный, ВидИнформации);
	...
	Индекс = ПочтовыйИндексАдреса(АдресНациональный);
	...
	Регион = АдресНациональный[ИмяСубъект];
	Если ПустаяСтрока(Регион) Тогда      
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, ИмяСубъект,
			НСтр("ru='Не указан регион.';uk='Не зазначений регіон.'"), "Регион");
	КонецЕсли;
	
	ЗданияПомещения = ЗданияИПомещенияАдреса(АдресНациональный);
	Если ИсключатьПроверкуДомаВАдресе(АдресНациональный) Тогда
	...
	Если Не ПустаяСтрока(Регион) Тогда
		Поле = "Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный();
	...
	ДополнительныйЭлемент = НайтиДополнительныйЭлементАдреса(АдресНациональный);
	...
	Если Не ИсключатьПроверкуУлицыВАдресе(АдресНациональный) Тогда
...
//
Функция ИсключатьПроверкуДомаВАдресе(Знач АдресНациональный)
	Результат = Ложь;
...
// Локальные исключения при проверке адреса.
//
Функция ИсключатьПроверкуУлицыВАдресе(Знач АдресНациональный)
	Результат = Ложь;
	
	// Дополнительные элементы адреса могут быть без улиц.
	ДополнительныеЭлементы = ЗначениеДополнительныхЭлементов(АдресНациональный);
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей = Истина)
	...
	ИмяСубъект = "Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный();
	ДобавитьЗначение(Список, "Регион", АдресНациональный[ИмяСубъект]);
	ДобавитьЗначение(Список, "КодРегиона", КодРегиона(АдресНациональный[ИмяСубъект]) );
	Если РасширенныйСоставПолей Тогда
		ДобавитьЗначение(Список, "РегионСокращение", УправлениеКонтактнойИнформациейКлиентСервер.Сокращение(АдресНациональный[ИмяСубъект]));
	КонецЕсли;
	...
	НаселПункт = АдресНациональный.НаселПункт;
	Улица = АдресНациональный.Улица;
	ДополнительныйЭлементАдреса = НайтиДополнительныйЭлементАдреса(АдресНациональный);
	...
	Иначе
		...
		ЗначениеКорпуса = "";
		Разделитель = "";
		Сокращения = УправлениеКонтактнойИнформациейКлиентСервер.СокращенияОбъектовАдресацииАдреса();
		Для Каждого СтрокаОбъекта Из ПараметрыОбъекта Цикл
...
Процедура ЗаполнитьОшибкиАдресаПоКлассификатору(XDTOАдрес, ФлагиПроверки, Результат)
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	НазваниеОсновнойСтраны  = "УКРАЇНА";
	ЭлементКвартира = Неопределено;
	ЭлементКорпус   = Неопределено;
	ЭлементДом      = Неопределено;
	
	АдресНациональный = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()));
	
	// Общий состав
	Адрес = Результат.Состав;          
	Адрес.Страна = НазваниеОсновнойСтраны;
	ЭтоНациональныйАдрес = Истина;
	...
	Для Каждого ЭлементСписка Из СписокПолей Цикл
		...
		Если ИмяПоля="ИНДЕКС" Тогда
			ЭлементИндекс = СоздатьДопАдрЭлемента(АдресНациональный);
		...
		ИначеЕсли ИмяПоля = "КОДРЕГИОНА" Тогда
			АдресНациональный["Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()] = РегионКода(ЭлементСписка.Значение);
			
		ИначеЕсли ИмяПоля = "РЕГИОН" Тогда
			АдресНациональный["Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()] = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "РАЙОН" Тогда
			Если АдресНациональный.СвРайМО = Неопределено Тогда
				АдресНациональный.СвРайМО = ФабрикаXDTO.Создать( АдресНациональный.Тип().Свойства.Получить("СвРайМО").Тип )
			КонецЕсли;
			АдресНациональный.СвРайМО.Район = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "ГОРОД" Тогда
			АдресНациональный.Город = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "НАСЕЛЕННЫЙПУНКТ" Тогда
			АдресНациональный.НаселПункт = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "УЛИЦА" Тогда
			АдресНациональный.Улица = ЭлементСписка.Значение;
			
		ИначеЕсли ИмяПоля = "ТИПДОМА" Тогда
			Если ЭлементДом = Неопределено Тогда
				ЭлементДом = СоздатьНомерДопАдрЭлемента(АдресНациональный);
			КонецЕсли;
		...
		ИначеЕсли ИмяПоля = "ТИПКОРПУСА" Тогда
			Если ЭлементКорпус = Неопределено Тогда
				ЭлементКорпус = СоздатьНомерДопАдрЭлемента(АдресНациональный);
			КонецЕсли;
		...
		ИначеЕсли ИмяПоля = "ТИПКВАРТИРЫ" Тогда
			Если ЭлементКвартира = Неопределено Тогда
				ЭлементКвартира = СоздатьНомерДопАдрЭлемента(АдресНациональный);
			КонецЕсли;
	...
	Адрес.Состав = ?(ЭтоНациональныйАдрес, АдресНациональный, Результат.Представление);
...
Функция ЕстьЗаполненныеСвойстваXDTOКонтактнойИнформации(Знач Владелец)
	...
	ИначеЕсли ТипВладельца = ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный()) Тогда
