...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.0.0.24";
	Обработчик.Процедура = "РегистрыСведений.РазмерыГосударственныхПособий.ЗаполнитьГосударственныеПособия";
	Обработчик.ОбщиеДанные = Истина;

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.0.9.3";
	Обработчик.Процедура = "РегистрыСведений.РазмерыГосударственныхПособий.ЗаполнитьГосударственныеПособия";
	Обработчик.ОбщиеДанные = Истина;
...
//~	Обработчик.Процедура = "ОбновлениеИнформационнойБазыЗарплатаКадрыРасширенный.ИсправитьНаборыЗначенийДоступа";
//-- ЕРП ЗИК	
...
//++ ЕРП ЗИК
//~Процедура ИсправитьНаборыЗначенийДоступа() Экспорт
//~	
//~	Если НЕ УправлениеДоступом.ОграничиватьДоступНаУровнеЗаписей() Тогда
//~		Возврат;
//~	КонецЕсли;	
//~	ОТ = Метаданные.ОпределяемыеТипы.Найти("ВладелецЗначенийКлючейДоступаОбъектЗарплатаКадры");
//~	Для Каждого ДокументТип из ОТ.Тип.Типы() Цикл
//~		ДокументМетаданные = Метаданные.НайтиПоТипу(ДокументТип);
//~		Менеджер = Документы[ДокументМетаданные.Имя];
...
//~				Продолжить;
//~			КонецЕсли;
//~			УправлениеДоступомСлужебный.ОбновитьНаборыЗначенийДоступа(ДокументОбъект,Ложь,Истина);
//~		КонецЦикла;	
//~	КонецЦикла;	
//~	
//~КонецПроцедуры
