...
Процедура РаспределитьПрочиеРасходыНезавершенногоПроизводства(ПараметрыРасчета)
	...
	|	ДД.АналитикаРасходов,
	|	ДД.Подразделение,
	|	СУММА(ВЫБОР КОГДА ДД.СлужебноеВидДвиженияПриход ТОГДА 1 ИНАЧЕ - 1 КОНЕЦ * ДД.СуммаРегл) КАК СуммаРегл,
    |	СУММА(ВЫБОР КОГДА ДД.СлужебноеВидДвиженияПриход ТОГДА 1 ИНАЧЕ - 1 КОНЕЦ * ДД.СуммаРеглБезНДС) КАК СуммаРеглБезНДС,    
    |	СУММА(ВЫБОР КОГДА ДД.СлужебноеВидДвиженияПриход ТОГДА 1 ИНАЧЕ - 1 КОНЕЦ * ДД.НДСРегл) КАК НДСРегл,
	|	СУММА(ВЫБОР КОГДА ДД.СлужебноеВидДвиженияПриход ТОГДА 1 ИНАЧЕ - 1 КОНЕЦ * ДД.ПостояннаяРазница) КАК ПостояннаяРазница,
	|	СУММА(ВЫБОР КОГДА ДД.СлужебноеВидДвиженияПриход ТОГДА 1 ИНАЧЕ - 1 КОНЕЦ * ДД.ВременнаяРазница) КАК ВременнаяРазница
	|ПОМЕСТИТЬ
	|	СуммыРасходов
	|ИЗ ВТКэшПрочиеРасходы КАК ДД
	|СГРУППИРОВАТЬ ПО
	|	ДД.Организация,
	|	ДД.СтатьяРасходов,
	|	ДД.АналитикаРасходов,
    ...
    |	МАКСИМУМ(ЕСТЬNULL(СуммыРасходов.СуммаРеглБезНДС, 0)) КАК СуммаРеглБезНДС,
    |	МАКСИМУМ(ЕСТЬNULL(СуммыРасходов.НДСРегл, 0)) КАК НДСРегл,
...
Процедура СформироватьУзлыКорректировкиСписанияСтоимостиРегл(ПараметрыРасчета)
	...
	|		Таблица.ВидЗапасов                 КАК ВидЗапасов,
	|		Таблица.Количество                 КАК Количество,
	|		Таблица.СуммаРегл                  КАК Стоимость,
	|		Таблица.СуммаРеглБезНДС            КАК СтоимостьРеглБезНДС,
	|		Таблица.НДСРегл                    КАК НДСРегл,
	|		0                                  КАК СтоимостьДопРасходы,
...
Процедура СписатьОшибкиОкругленияРасчетаСебестоимости(ПараметрыРасчета)
    ...
    |   ВЫБОР 
    |       КОГДА 
	|			НЕ УчетнаяПолитикаОрганизаций.ПлательщикНДС
	...
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&КонецПериода,) КАК УчетнаяПолитикаОрганизаций
    |;
    |ВЫБРАТЬ
    //-- НЕ УТ
    ...
    //++ НЕ УТ
    |	ЕСТЬNULL(НалоговыеНазначенияОрганизаций.НалоговоеНазначение, ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Облагаемая)) КАК НалоговоеНазначение,
    //-- НЕ УТ
    ...
    //++ НЕ УТ
	|	    ЛЕВОЕ СОЕДИНЕНИЕ НалоговыеНазначенияОрганизаций КАК НалоговыеНазначенияОрганизаций
	|	    ПО Т.Организация = НалоговыеНазначенияОрганизаций.Организация
    //-- НЕ УТ
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.ДокументДвижения,
	|	Т.Организация,
    //++ НЕ УТ
    |	ЕСТЬNULL(НалоговыеНазначенияОрганизаций.НалоговоеНазначение, ЗНАЧЕНИЕ(Справочник.НалоговыеНазначенияАктивовИЗатрат.НДС_Облагаемая)),
    //-- НЕ УТ
	|	Т.Подразделение
	|
	|ИМЕЮЩИЕ
