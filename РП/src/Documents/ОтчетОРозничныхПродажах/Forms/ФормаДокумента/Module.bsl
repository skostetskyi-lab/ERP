...
&НаКлиенте
Процедура СуммаОплатыНаличныхПриИзменении(Элемент)
	Объект.СуммаОкругления = СуммаОплатыНаличных
...
Процедура ВыручкаПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ЗаполнитьСуммуОкругленияПоОтчетамОВыручке();
КонецПроцедуры

&НаКлиенте
Процедура ВыручкаПослеУдаления(Элемент)
	ЗаполнитьСуммуОкругленияПоОтчетамОВыручке();
КонецПроцедуры
...
Процедура ЗаполнитьВыручку(Команда)
	...
	Иначе
		...
		ДатаНач = Объект.НачалоПериода;
		ДатаКон = КонецДня(Объект.ОкончаниеПериода);
...
Процедура ОбновитьВидимостьЭлементов(КассаФискальныйРегистратор)
	...
	Элементы.СтраницаВыручка.Видимость = КассаАвтономнаяККМ;
	Элементы.ВыручкаСумма.Видимость = ОрганизацияПлательщикНДС;
	Элементы.ВыручкаСтавкаНДС.Видимость = ОрганизацияПлательщикНДС;
...
Процедура РассчитатьИтоговыеПоказателиОтчетаОРозничныхПродажах(Форма)
	...
	Форма.СуммаОплатыНаличных = Форма.СуммаВсего + Форма.Объект.СуммаОкругления
		- Форма.Объект.ОплатаПлатежнымиКартами.Итог("Сумма")
		- Форма.Объект.ПодарочныеСертификаты.Итог("Сумма");
...
Процедура ПересчитатьСуммуПоЕдиномуНалогу()
	...
	ДенежныеСредстваСервер.ПересчетСуммыПоЕдиномуНалогу(Объект, СуммаВсего + Объект.СуммаОкругления);
...
Процедура ЗаполнитьВыручкуНаСервере(ДатаНач, ДатаКон)
	...
	Объект.НачалоПериода = ДатаНач;
	Объект.ОкончаниеПериода = ДатаКон;
	ЗаполнитьПредставлениеПериода();
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОтчетОРозничнойВыручке.Ссылка,
	|	СУММА(ОтчетОРозничнойВыручке.СуммаОкругления) КАК СуммаОкругления
	|ПОМЕСТИТЬ ОРВ
	|ИЗ
	|	Документ.ОтчетОРозничнойВыручке КАК ОтчетОРозничнойВыручке
	|ГДЕ
	|	ОтчетОРозничнойВыручке.Дата МЕЖДУ &ДатаНач И &ДатаКон
	|	И ОтчетОРозничнойВыручке.Проведен
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОРВ.Ссылка КАК ОтчетОРозничнойВыручке,
	...
	|ИЗ
	|	ОРВ КАК ОРВ
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСРасчетНалоговыхОбязательств КАК НДСРасчетНалоговыхОбязательств
	|		ПО ОРВ.Ссылка = НДСРасчетНалоговыхОбязательств.Регистратор
	|			И (НДСРасчетНалоговыхОбязательств.Период МЕЖДУ &ДатаНач И &ДатаКон)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ОРВ.СуммаОкругления) КАК СуммаОкругления
	|ИЗ
	|	ОРВ КАК ОРВ";
	
	Результат = Запрос.ВыполнитьПакет();
	Объект.Выручка.Загрузить(Результат[1].Выгрузить());	  
	ВыборкаШапка = Результат[2].Выбрать();
	ВыборкаШапка.Следующий();
	Объект.СуммаОкругления = ВыборкаШапка.СуммаОкругления;
	РассчитатьИтоговыеПоказателиОтчетаОРозничныхПродажах(ЭтаФорма);
	ПересчитатьСуммуПоЕдиномуНалогу();
...
Процедура ЗаполнитьПредставлениеПериода()
	...
	Если ЗначениеЗаполнено(Объект.НачалоПериода) Тогда
		...
		НадписьПериодЗаполнения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выручка заполнена за %1–%2';uk='Виручка заповнена за %1-%2'"), 
			Формат(Объект.НачалоПериода, "ДЛФ=Д"), Формат(Объект.ОкончаниеПериода, "ДЛФ=Д"));	
		
	Иначе
		НадписьПериодЗаполнения = "";
	КонецЕсли;
	
КонецПроцедуры 

&НаСервере
Процедура ЗаполнитьСуммуОкругленияПоОтчетамОВыручке()
	...
	|	Документ.ОтчетОРозничнойВыручке КАК ОтчетОРозничнойВыручке
	|ГДЕ
	|	ОтчетОРозничнойВыручке.Ссылка В (&МассивОРВ)";
	Запрос.УстановитьПараметр("МассивОРВ", Объект.Выручка.Выгрузить(,"ОтчетОРозничнойВыручке").ВыгрузитьКолонку(0)); 
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	Объект.СуммаОкругления = Выборка.СуммаОкругления;
	РассчитатьИтоговыеПоказателиОтчетаОРозничныхПродажах(ЭтаФорма);
	ПересчитатьСуммуПоЕдиномуНалогу();
