...
#Область ТекстСебестоимостьРеализованногоТовара // (Дт 90 :: Кт 28)
	...
	|
	|	0 КАК ВалютнаяСуммаКт,
    |	СУММА(Строки.Знак * Строки.Количество) КАК КоличествоКт,
	|	0 КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
...
#Область ТекстВычетИзДоходовОтВозвратаСобственногоТовараАкциз // (Дт 70 :: Кт З6Р)
	ТекстВычетИзДоходовОтВозвратаСобственногоТовараАкциз = "
	|ВЫБРАТЬ //// Вычет из доходов от продаж в рознице акциз (возврат) (Дт 70 :: Кт 36Р)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК СуммаВРКт,
	|
	|	""Вычет из доходов от продаж в рознице акциз (возврат)"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстАкцизВозвратТоваров // (Дт 70 :: Кт 6416)
	ТекстАкцизВозвратТоваров = "
...
#Область ТекстОкруглениеНаличныхДоходы // (Дт 308 :: Кт 70)
	ТекстОкруглениеНаличныхДоходы = "
	|ВЫБРАТЬ //// Округление наличных в рознице, доход (Дт 308 :: Кт 70)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|   НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	Операция.СуммаОкругления КАК Сумма,
	|
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	...
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ОкруглениеНаличныхПриРозничныхПродажах) КАК АналитикаУчетаКт,
	|	Операция.Подразделение КАК МестоУчетаКт,
	|
	|	Операция.Валюта КАК ВалютаКт,
	|	Операция.Подразделение КАК ПодразделениеКт,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""Округление наличных при розничных продажах"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
#Область ТекстОкруглениеНаличныхРасходы // (Дт 9X :: Кт 308)
	ТекстОкруглениеНаличныхРасходы = "
	|ВЫБРАТЬ //// Округление наличных в рознице, расходы (Дт 9X :: Кт 308)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ОкруглениеНаличныхПриРозничныхПродажах) КАК АналитикаУчетаДт,
	...
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ОкруглениеНаличныхПриРозничныхПродажах) КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
...
#КонецОбласти 
	...
	Возврат ТекстВыручкаОтРеализацииСобственногоТовара
        ...
        + " ОБЪЕДИНИТЬ ВСЕ " + ТекстАкцизВозвратТоваров
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОкруглениеНаличныхДоходы
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстОкруглениеНаличныхРасходы
		;
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваКонтрагент(Запрос, ТекстыЗапроса, Регистры)
	...
	|	
	|	ВЫРАЗИТЬ((ДанныеДокумента.СуммаДокумента - ДанныеДокумента.СуммаОкругления - ЕСТЬNULL(ВтСуммаПлатежнымиКартами.Сумма, 0))
	...
	|	ДанныеДокумента.СуммаДокумента - ДанныеДокумента.СуммаОкругления - ЕСТЬNULL(ВтСуммаПлатежнымиКартами.Сумма, 0) КАК СуммаОплатыВВалютеПлатежа,
	|
	...
	|
	|	ДанныеДокумента.СуммаДокумента - ДанныеДокумента.СуммаОкругления - ЕСТЬNULL(ВтСуммаПлатежнымиКартами.Сумма, 0) КАК СуммаОплатыВВалютеВзаиморасчетов,
	...
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ДанныеДокумента.СуммаДокумента - ДанныеДокумента.СуммаОкругления - ЕСТЬNULL(ВтСуммаПлатежнымиКартами.Сумма, 0) <> 0
	|	И ДанныеДокумента.Ссылка = &Ссылка
	|	И &ПолноеПроведение
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Функция ТекстЗапросаТаблицаПрочиеДоходы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	&Организация КАК Организация,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ОкруглениеНаличныхПриРозничныхПродажах) КАК СтатьяДоходов,
	|	&Организация КАК АналитикаДоходов,
	|	&Подразделение КАК Подразделение,
	|	
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОкруглениеНаличныхПриРозничныхПродажах) КАК ХозяйственнаяОперация,
	|	
	|	ДанныеДокумента.СуммаОкругления КАК СуммаРегл,
	|	ВЫРАЗИТЬ(ДанныеДокумента.СуммаОкругления * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15,2)) КАК Сумма
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка 
	|	И ДанныеДокумента.СуммаОкругления > 0
	|	
	|";
...
Функция ТекстЗапросаТаблицаПрочиеРасходы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	&Организация КАК Организация,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ОкруглениеНаличныхПриРозничныхПродажах) КАК СтатьяРасходов,
	|	&Организация КАК АналитикаРасходов,
	|	&Подразделение КАК Подразделение,
    |	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОкруглениеНаличныхПриРозничныхПродажах) КАК ХозяйственнаяОперация,
	|	0 КАК СуммаБезНДС,
	|	-ДанныеДокумента.СуммаОкругления КАК СуммаРегл,
	|	-ДанныеДокумента.СуммаОкругления КАК СуммаРеглБезНДС,
	|	0 КАК НДСРегл,
...
Функция ТекстЗапросаДвиженияДенежныеСредстваДоходыРасходы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеДокумента.Валюта КАК Валюта,
	|
	|	ВЫБОР КОГДА ДанныеДокумента.СуммаОкругления > 0 ТОГДА
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ОкруглениеНаличныхПриРозничныхПродажах)
	|	ИНАЧЕ
	|		ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ОкруглениеНаличныхПриРозничныхПродажах)
	|	КОНЕЦ КАК СтатьяДоходовРасходов,
	|	ДанныеДокумента.Организация                           КАК АналитикаДоходов,
	|	ДанныеДокумента.Организация                           КАК АналитикаРасходов,
	|
	|	ВЫБОР КОГДА ДанныеДокумента.СуммаОкругления > 0 ТОГДА
	|		ВЫРАЗИТЬ(ДанныеДокумента.СуммаОкругления * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15,2))
	|	ИНАЧЕ
	|		- ВЫРАЗИТЬ(ДанныеДокумента.СуммаОкругления * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15,2))
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР КОГДА ДанныеДокумента.СуммаОкругления > 0 ТОГДА
	|		ДанныеДокумента.СуммаОкругления
	|	ИНАЧЕ
	|		-ДанныеДокумента.СуммаОкругления
	|	КОНЕЦ КАК СуммаРегл,
	|	ВЫБОР КОГДА ДанныеДокумента.СуммаОкругления > 0 ТОГДА
