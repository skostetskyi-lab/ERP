...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КратностьОкругленияСуммыНаличныхВЧеке = Константы.КратностьОкругленияСуммыНаличныхВЧеке.Получить();
...
&НаКлиенте
Процедура ИзменитьОтключитьОкруглениеСуммыНаличных(Команда)
	ОтключитьОкруглениеСуммыНаличных = Не ОтключитьОкруглениеСуммыНаличных;
	Элементы.ОтключитьОкруглениеСуммыНаличных.Пометка = ОтключитьОкруглениеСуммыНаличных;
КонецПроцедуры
...
Функция ПараметрыЧека()
    ...
    ИнформацияОбОплате = ИнформацияОбОплатеДляПробитияЧека(Объект.Ссылка);
	ТаблицаОплат = ИнформацияОбОплате.ТаблицаОплат; 
...
&НаКлиентеНаСервереБезКонтекста
Функция ОкругленнаяСуммаНаличных(СуммаНаличные, Кратность)
	
	Если Кратность = 0 Тогда
		Возврат СуммаНаличные;
	Иначе
		Возврат Окр(СуммаНаличные / Кратность, 0) * Кратность;
	КонецЕсли;
...
Функция ИнформацияОбОплате(Форма)
	...
	ИнформацияОбОплате.Вставить("КратностьОкругленияСуммыНаличныхВЧеке", ?(Форма.ОтключитьОкруглениеСуммыНаличных, 0, Форма.КратностьОкругленияСуммыНаличныхВЧеке));
	
	КОплатеНаличными = СуммаКОплате - ИнформацияОбОплате.ИтогоОплачено + ИнформацияОбОплате.Наличные;
	КОплатеНаличнымиОкругленная = ОкругленнаяСуммаНаличных(КОплатеНаличными, ИнформацияОбОплате.КратностьОкругленияСуммыНаличныхВЧеке);
	СуммаОкругления = КОплатеНаличнымиОкругленная - КОплатеНаличными;
	
	ИнформацияОбОплате.Вставить("СуммаКОплатеОкругленная", СуммаКОплате + СуммаОкругления);
	ИнформацияОбОплате.Вставить("СуммаОкругления", СуммаОкругления);	
...
&НаСервереБезКонтекста
Функция ИнформацияОбОплатеДляПробитияЧека(Ссылка)
	...
	|	ОплатаПлатежнымиКартами.ЭквайринговыйТерминал КАК СредствоОплаты,
	...
	|ИЗ
	|	Документ.ЧекККМ.ПодарочныеСертификаты КАК ПодарочныеСертификаты
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СредстваОплатыДляФискальныхЧеков.Название,
	|	СредстваОплатыДляФискальныхЧеков.Код,
	|	СредстваОплаты.СредствоОплаты,
	|	0 КАК Приоритет
	|ПОМЕСТИТЬ ВариантыНазваний
	|ИЗ
	|	СредстваОплаты КАК СредстваОплаты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СредстваОплатыДляФискальныхЧеков КАК СредстваОплатыДляФискальныхЧеков
	|		ПО СредстваОплаты.СредствоОплаты = СредстваОплатыДляФискальныхЧеков.СредствоОплаты
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	СредстваОплаты КАК СредстваОплаты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СредстваОплатыДляФискальныхЧеков КАК СредстваОплатыДляФискальныхЧеков
	|		ПО (ТИПЗНАЧЕНИЯ(СредстваОплатыДляФискальныхЧеков.СредствоОплаты) = ТИПЗНАЧЕНИЯ(СредстваОплаты.СредствоОплаты))
	|ГДЕ
	|	(СредстваОплатыДляФискальныхЧеков.СредствоОплаты = ЗНАЧЕНИЕ(Справочник.ЭквайринговыеТерминалы.ПустаяСсылка)
	|			ИЛИ СредстваОплатыДляФискальныхЧеков.СредствоОплаты = ЗНАЧЕНИЕ(Справочник.ВидыПодарочныхСертификатов.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВариантыНазваний.Название КАК НазваниеСредстваОплаты,
	|	ВариантыНазваний.Код КАК КодСредстваОплаты,
	|	СредстваОплаты.ТипОплаты,
	|	СУММА(СредстваОплаты.Сумма) КАК Сумма,
	|	СредстваОплаты.СредствоОплаты КАК СредствоОплаты,
	|	СредстваОплаты.НомерПлатежнойКарты,
	|	СредстваОплаты.СсылочныйНомер,
	|	СредстваОплаты.КодАвторизации,
	|	СредстваОплаты.НомерЧекаЭТ,
	|	СредстваОплаты.ПодарочныйСертификат
	|ИЗ
	|	СредстваОплаты КАК СредстваОплаты
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВариантыНазваний КАК ВариантыНазваний
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|				ВариантыНазваний.СредствоОплаты КАК СредствоОплаты,
	|				МИНИМУМ(ВариантыНазваний.Приоритет) КАК Приоритет
	|			ИЗ
	|				ВариантыНазваний КАК ВариантыНазваний
	|			
	|			СГРУППИРОВАТЬ ПО
	|				ВариантыНазваний.СредствоОплаты) КАК ВложенныйЗапрос
	|			ПО ВариантыНазваний.СредствоОплаты = ВложенныйЗапрос.СредствоОплаты
	|				И ВариантыНазваний.Приоритет = ВложенныйЗапрос.Приоритет
	|		ПО СредстваОплаты.СредствоОплаты = ВариантыНазваний.СредствоОплаты
	|
	|СГРУППИРОВАТЬ ПО
	|	СредстваОплаты.СсылочныйНомер,
	|	СредстваОплаты.НомерПлатежнойКарты,
	|	СредстваОплаты.СредствоОплаты,
	|	ВариантыНазваний.Название,
	|	СредстваОплаты.КодАвторизации,
	|	СредстваОплаты.НомерЧекаЭТ,
	|	СредстваОплаты.ПодарочныйСертификат,
	|	ВариантыНазваний.Код,
	|	СредстваОплаты.ТипОплаты";
	...
	ВыборкаОплаты = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выбрать(); 
	
	ТаблицаОплат = Новый Массив; 
	
	Если ЗначениеЗаполнено(РеквизитыДокумента.ПолученоНаличными) Тогда
		
		СтрокаОплаты = Новый СписокЗначений();
		СтрокаОплаты.Добавить(0);
		СтрокаОплаты.Добавить(РеквизитыДокумента.ПолученоНаличными);
		СтрокаОплаты.Добавить("Готівка");
		СтрокаОплаты.Добавить(0);
		
		СтруктураДопСвойств = Новый Структура;
		Если ЗначениеЗаполнено(РеквизитыДокумента.СуммаОкругления) Тогда
			СтруктураДопСвойств.Вставить("СуммаОкругления", РеквизитыДокумента.СуммаОкругления);	
			СтруктураДопСвойств.Вставить("СуммаДокумента", РеквизитыДокумента.СуммаДокумента);	
		КонецЕсли;
		СтрокаОплаты.Добавить(СтруктураДопСвойств);
	...
	Пока ВыборкаОплаты.Следующий() Цикл
		...
		НазваниеСредстваОплаты = "";
		Если ЗначениеЗаполнено(ВыборкаОплаты.НазваниеСредстваОплаты) Тогда
			НазваниеСредстваОплаты = ВыборкаОплаты.НазваниеСредстваОплаты;
		ИначеЕсли ВыборкаОплаты.ТипОплаты = 1 Тогда
			НазваниеСредстваОплаты = "Картка";
		ИначеЕсли ВыборкаОплаты.ТипОплаты = 3 Тогда
			НазваниеСредстваОплаты = "Подарунковий сертифікат";
		КонецЕсли;
		
		СтрокаОплаты = Новый СписокЗначений();
		СтрокаОплаты.Добавить(ВыборкаОплаты.ТипОплаты);
		СтрокаОплаты.Добавить(ВыборкаОплаты.Сумма);
		СтрокаОплаты.Добавить(НазваниеСредстваОплаты);
		СтрокаОплаты.Добавить(ВыборкаОплаты.КодСредстваОплаты);
		
		СтруктураДопСвойств = Новый Структура;
		Если ВыборкаОплаты.ТипОплаты = 1 Тогда
			СтруктураДопСвойств	= Новый Структура("НомерПлатежнойКарты, СсылочныйНомер, НомерЧекаЭТ, КодАвторизации");	
			ЗаполнитьЗначенияСвойств(СтруктураДопСвойств, ВыборкаОплаты);
		КонецЕсли;
		СтрокаОплаты.Добавить(СтруктураДопСвойств);
	...
	ИнформацияОбОплате = Новый Структура;
	ИнформацияОбОплате.Вставить("СуммаДокумента",        РеквизитыДокумента.СуммаДокумента);
	ИнформацияОбОплате.Вставить("СуммаОкругления", 		 РеквизитыДокумента.СуммаОкругления);	
	ИнформацияОбОплате.Вставить("ТаблицаОплат", 		 ТаблицаОплат);	
...
Процедура ОбработатьДобавлениеОплаты(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры = Неопределено Тогда
		...
		Если ИнформацияОбОплате.ИтогоОплачено >= ИнформацияОбОплате.СуммаКОплатеОкругленная Тогда	
			
			Объект.СуммаОкругления = ИнформацияОбОплате.СуммаОкругления;
...
Процедура ЗагрузитьНовыйЧекФрагмент()
	...
	ОтключитьОкруглениеСуммыНаличных = Ложь;
	Элементы.ОтключитьОкруглениеСуммыНаличных.Пометка = ОтключитьОкруглениеСуммыНаличных;
	...
	Если ОтобразитьСдачу Тогда
		...
		ТекстСдача = Новый ФорматированнаяСтрока(
			СтрШаблон(НСтр("ru='Сдача: %1';uk='Здача: %1'"),   
				Формат(ИнформацияОбОплате.ИтогоОплачено - ИнформацияОбОплате.СуммаКОплатеОкругленная, "ЧДЦ=2; ЧН=0.00")));
...
Процедура НастроитьРМК()
	...
	|	НастройкиРМК.РазрешитьОтключениеОкругления              		  КАК РазрешитьОтключениеОкругления,
	...
	Если Выборка.Следующий() Тогда
		...
		РазрешитьОтключениеОкругления  = Выборка.РазрешитьОтключениеОкругления; 
	...
	Иначе
		...
		РазрешитьОтключениеОкругления = Ложь;
	...
	Элементы.ОтключитьОкруглениеСуммыНаличных.Видимость = РазрешитьОтключениеОкругления;
