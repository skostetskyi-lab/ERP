...
Функция ПечатнаяФормаПриказаОПредоставленииОтгула(Макет, МассивОбъектов, ОбъектыПечати, КодЯзыка)
	...
	Пока Выборка.Следующий() Цикл 
		...
		Если НастройкиПечатныхФорм.УдалятьПрефиксыОрганизацииИИБИзНомеровКадровыхПриказов Тогда
			ОбластьШапка.Параметры.НомерДок = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СокрЛП(Выборка.НомерДок), Истина, Истина);
		КонецЕсли;
		...
		ПредставлениеДней   = НСтр("ru='день,дня,дней';uk='день,дні,днів'",КодЯзыка);
		ПредставлениеЧасов  = НСтр("ru='час,часа,часов';uk='година,години,годин'",КодЯзыка);
		
		ОбластьШапка.Параметры.ДнейЧасов = ?(Выборка.ОтсутствиеВТечениеЧастиСмены, 
			СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(Выборка.КоличествоЧасовОтгула, ПредставлениеЧасов), 
			СтроковыеФункцииКлиентСервер.ЧислоЦифрамиПредметИсчисленияПрописью(Выборка.КоличествоДнейОтгула, ПредставлениеДней));
