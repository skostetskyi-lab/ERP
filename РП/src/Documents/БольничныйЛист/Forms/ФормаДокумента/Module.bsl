...
Процедура ДатаНачалаПриИзменении(Элемент)
	...
	ДатаНачалаДоИзменения = Объект.ДатаНачалаСобытия; 

	Если Не Объект.ЯвляетсяПродолжениемБолезни И Объект.РучнаяКорректировкаСреднегоЗаработка И Месяц(Объект.ДатаНачала) <> Месяц(ДатаНачалаДоИзменения) Тогда
		Оповещение = Новый ОписаниеОповещения("ДатаНачалаПродолжение", ЭтотОбъект, Новый Структура("ДатаНачалаДоИзменения", ДатаНачалаДоИзменения));
		ПоказатьВопрос(Оповещение, НСтр("ru='Ручные исправления таблиц документа будут потеряны. Продолжить?';uk='Ручні виправлення таблиць документа будуть втрачені. Продовжити?'"), РежимДиалогаВопрос.ОКОтмена); 
	Иначе 
...
&НаКлиенте
Процедура ДатаНачалаПродолжение(Ответ, ДополнительныеПараметры) Экспорт
	
	Если Ответ = КодВозвратаДиалога.ОК Тогда
		Объект.РучнаяКорректировкаСреднегоЗаработка = Ложь;  
	Иначе	 
		Объект.ДатаНачала = ДополнительныеПараметры.ДатаНачалаДоИзменения;       
	КонецЕсли;
	ДатаНачалаПриИзмененииНаСервере();
	ПриИзмененииДатыОсвобожденияОтРаботы()
