...
//~	СведенияОбОрганизации = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(МассивОбъектов.Организация, МассивОбъектов.Дата);
//-- ЕРП ЗИК	
	...
	Для Каждого Строка ИЗ РезультатЗапросаПоСтрокам Цикл
		...
		Если ЗначениеЗаполнено(Строка.Звание) Тогда
			ОбластьСтрока.Параметры.ЗваниеПрофиль	= Строка.Звание + ", " + Строка.ВУС + ?(ЗначениеЗаполнено(Строка.Профиль), " ("+Локализация.ПолучитьЛокализованноеПредставление(Строка.Профиль, "uk")+")", "");
		Иначе
