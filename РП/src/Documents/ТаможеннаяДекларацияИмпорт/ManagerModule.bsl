...
#Область ТекстНДСНаСебестоимостьЗакупкаПодНеоблагаемуюДеятельность // (Дт 28 :: Кт 63)
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|                           
	|	ВЫБОР КОГДА Строки.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) ТОГДА
...
#Область ТекстНДСНаРасходыЗакупкаПодНеоблагаемуюДеятельность // (Дт 9X :: Кт 63)
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|  
	|	СУММА(ВЫБОР КОГДА Строки.НалоговоеНазначение.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиНДС.ПропорциональноОблагаемая) ТОГДА
	|   	Строки.СуммаНДСПропорционально
	|   ИНАЧЕ	
	|		Строки.СуммаНДС
	|   КОНЕЦ) КАК Сумма,
