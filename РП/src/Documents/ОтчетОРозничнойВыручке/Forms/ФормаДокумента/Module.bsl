...
&НаКлиенте
Процедура СуммаОплатыНаличнымиПриИзменении(Элемент)
	Объект.СуммаОкругления = СуммаОплатыНаличными + Объект.СуммаОплатыПлатежнымиКартами - СуммаВсего;	
	ПересчитатьСуммуПоЕдиномуНалогу();
КонецПроцедуры
...
Процедура ЗаполнитьПоОтчетуОРозничныхПродажах(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.КассаККМ) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не указана касса ККМ';uk='Не зазначена каса ККМ'"));
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Склад) Тогда
		ПоказатьПредупреждение(,НСтр("ru='Не указан склад';uk='Не зазначений склад'"));
		Возврат;
	КонецЕсли;

	Если Объект.Выручка.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоОтчетуОРозничныхПродажахПродолжение", ЭтотОбъект), НСтр("ru='Перед заполнением табличная часть будет очищена, продолжить?';uk='Перед заповненням таблична частина буде очищена, продовжити?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьПоОтчетуОРозничныхПродажахВыбратьДокумент();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоОтчетуОРозничныхПродажахПродолжение(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗаполнитьПоОтчетуОРозничныхПродажахВыбратьДокумент() Экспорт
	...
	ОткрытьФорму("Документ.ОтчетОРозничныхПродажах.Форма.ФормаВыбора", Новый Структура("Отбор", СтруктураОтбора), ЭтаФорма,,,, Новый ОписаниеОповещения("ЗаполнитьПоОтчетуОРозничныхПродажахЗавершение",ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоОтчетуОРозничныхПродажахЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура РассчитатьИтоговыеПоказатели(Форма)
	...
	Форма.СуммаОплатыНаличными = СуммаВсего - Форма.Объект.СуммаОплатыПлатежнымиКартами + Форма.Объект.СуммаОкругления;
