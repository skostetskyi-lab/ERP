...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого СтрокаВзносы Из ВзносыФОТ Цикл
		...
		Если СтрокаВзносы.МесяцРасчетногоПериода >= ПериодРегистрации Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='В строке %1, табличной части ""Взносы ФОТ"" налоговый период должен быть меньше периода регистрации!';uk='У рядку %1, табличної частини ""Внески ФОП"" податковий період повинен бути менше періоду реєстрації!'"),
		...
		Если ОснованиеДляДоначисления = Перечисления.ОснованияДляДоначисленияСтраховыхВзносов.ДляИсправленияОшибок И СтрокаВзносы.ТипНачисления > 3 
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='В строке %1, табличной части ""Взносы ФОТ"" не верно указан тип начисления!';uk='У рядку %1, табличної частини ""Внески ФОП"" не вірно вказано тип нарахування!'"),
						СтрокаВзносы.НомерСтроки);
	...
	Для каждого СтрокаВзносы Из Взносы Цикл
		...
		Если СтрокаВзносы.МесяцРасчетногоПериода >= ПериодРегистрации Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='В строке %1, табличной части ""Взносы"" налоговый период должен быть меньше периода регистрации!';uk='У рядку %1, табличної частини ""Внески"" податковий період повинен бути менше періоду реєстрації!'"),
						СтрокаВзносы.НомерСтроки);
		...
		Если ОснованиеДляДоначисления = Перечисления.ОснованияДляДоначисленияСтраховыхВзносов.ДляИсправленияОшибок И СтрокаВзносы.ТипНачисления > 3 
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='В строке %1, табличной части ""Взносы"" не верно указан тип начисления!';uk='У рядку %1, табличної частини ""Внески"" не вірно вказано тип нарахування!'"),
						СтрокаВзносы.НомерСтроки);
