...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_UK_ЛистокСообщения2025") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
				КоллекцияПечатныхФорм,
				"ПФ_MXL_UK_ЛистокСообщения2025", НСтр("ru='Листок сообщения';uk='Листок повідомлення'"),
				ПечатнаяФормаЛисткаСообщения2025(МассивОбъектов, ОбъектыПечати), ,
				"Документ.ЛистокСообщенияДляВоенкомата.ПФ_MXL_UK_ЛистокСообщения2025");
	КонецЕсли;
...
Функция ПечатнаяФормаЛисткаСообщения2025(МассивОбъектов, ОбъектыПечати)
	
	НастройкиПечатныхФорм = ЗарплатаКадрыПовтИсп.НастройкиПечатныхФорм();
	
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЛистокСообщения2025";
	
	ТабДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЛистокСообщенияДляВоенкомата.ПФ_MXL_UK_ЛистокСообщения2025");
	
	ОбластьМакетаЛисток = Макет.ПолучитьОбласть("Листок"); 
	
	ДанныеДляПечати = ДанныеДляПечатиЛисткаСообщения2025(МассивОбъектов).Выбрать();
...
//~		ОтветственныеЛица = ОтветственныеЛицаБППовтИсп.ОтветственныеЛица(ДанныеДляПечати.Организация, ДанныеДляПечати.Дата);
//-- ЕРП ЗИК				
		...
		Если ТипЗнч(ДанныеДляПечати.ДатаВыдачи) = Тип("Дата") и ЗначениеЗаполнено(ДанныеДляПечати.ДатаВыдачи) Тогда
			ОбластьМакетаЛисток.Параметры.ДатаВыдачи = Формат(ДанныеДляПечати.ДатаВыдачи, "Л=uk_UA; ДЛФ=DD"); 
		КонецЕсли;
		
		Если ТипЗнч(ДанныеДляПечати.ДатаВремяФормирования) = Тип("Дата") и ЗначениеЗаполнено(ДанныеДляПечати.ДатаВремяФормирования)  Тогда
			ОбластьМакетаЛисток.Параметры.ДатаВремяФормирования = Формат(ДанныеДляПечати.ДатаВремяФормирования, "ДФ='ЧЧ:мм дд.ММ.гггг'") + " р."; 
		КонецЕсли;
		...
		МассивДлинСтрок.Установить(0,30);
		...
		КонецЦикла;
		РезультирующаяСтрокаАдрес = ЗарплатаКадры.РазбитьСтрокуНаПодСтроки(ДанныеДляПечати.АдресПрописки, МассивДлинСтрок);
		Для Счетчик = 1 По ?(СтрЧислоСтрок(РезультирующаяСтрокаАдрес) <= 2, СтрЧислоСтрок(РезультирующаяСтрокаАдрес), 2)  Цикл
			ОбластьМакетаЛисток.Параметры["АдресПрописки"+Счетчик] = СтрЗаменить(СтрПолучитьСтроку(РезультирующаяСтрокаАдрес, Счетчик),Символы.ПС, "");  
		КонецЦикла;
...
Функция ДанныеДляПечатиЛисткаСообщения2025(МассивСсылок)
	...
	|	ЛистокСообщенияДляВоенкомата.АдресПоПропискеПредставление КАК АдресПрописки,
	...
	|	ДанныеЛистка.Ссылка,
	|	МАКСИМУМ(ВоинскийУчет.Период) КАК ПериодВУ,
	|	МАКСИМУМ(ДокументыФизическихЛиц.Период) КАК ПериодДФЛ
	|ПОМЕСТИТЬ Периоды
	|ИЗ
	...
	|		ПО ДанныеЛистка.ФизическоеЛицо = ДокументыФизическихЛиц.Физлицо
	|			И ДанныеЛистка.Дата >= ДокументыФизическихЛиц.Период
	...
	|	ЕСТЬNULL(ДокументыФизическихЛиц.ДатаВремяФормирования, """") КАК ДатаВремяФормирования,
	|	ДанныеЛистка.Ссылка,
	|	ДанныеЛистка.ФизическоеЛицо,
	...
	|		ПО ДанныеЛистка.ФизическоеЛицо = Периоды.ФизическоеЛицо
	|		 И ДанныеЛистка.Ссылка = Периоды.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВоинскийУчет КАК ВоинскийУчет
	|		ПО ДанныеЛистка.ФизическоеЛицо = ВоинскийУчет.ФизическоеЛицо
	|			И Периоды.ПериодВУ = ВоинскийУчет.Период
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыФизическихЛиц КАК ДокументыФизическихЛиц
	|		ПО ДанныеЛистка.ФизическоеЛицо = ДокументыФизическихЛиц.Физлицо
	|			И Периоды.ПериодДФЛ = ДокументыФизическихЛиц.Период
	|			И ДокументыФизическихЛиц.ВидДокумента = ЗНАЧЕНИЕ(Справочник.ВидыДокументовФизическихЛиц.ВоенныйБилет)
	|" ;
...
Функция ФиксацияОписаниеФиксацииРеквизитов()
	...
	ОписаниеФиксацииРеквизитов.Вставить("АдресПоПропискеПредставление",		 	ФиксацияОписаниеФиксацииРеквизита("АдресПоПропискеПредставление", "ГруппаИнформацияПраваяКолонка", "Сотрудник", НСтр("ru='Адрес заполняется автоматически. Если вы хотите исправить данные, нажмите ""Нет"" и затем используйте ссылку ""Редактировать карточку сотрудника"". Если вы хотите изменить адрес только в этом документе, нажмите ""Да"" и продолжите редактирование';uk='Адреса заповнюється автоматично. Якщо ви хочете виправити дані, натисніть кнопку ""Ні"", а потім скористайтеся посиланням ""Редагувати картку співробітника"". Якщо ви хочете змінити адресу тільки в цьому документі, натисніть ""Так"" і продовжіть редагування'")));
