...
Процедура ЗаполнитьЮридическийАдрес(ОбъектXDTO)
	...
	Если ОбъектXDTO.СвАдрес <> Неопределено
		...
		СубъектИмя = "Субъект"+УправлениеКонтактнойИнформациейКлиентСервер.СуффиксНациональный();
		Если ОбъектXDTO.СвАдрес.Адрес.Состав[СубъектИмя] <> Неопределено Тогда
			Регион = ОбъектXDTO.СвАдрес.Адрес.Состав[СубъектИмя];
		КонецЕсли;
