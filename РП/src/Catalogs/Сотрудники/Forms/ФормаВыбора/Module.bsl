...
//-- ЕРП ЗИК	
	...
	ВидЗанятостиОтбор = Неопределено;
	Если Параметры.Отбор.Свойство("ВидЗанятости", ВидЗанятостиОтбор) Тогда
		
		    Если ТипЗнч(ВидЗанятостиОтбор) = Тип("СписокЗначений") Тогда
				ВидСравненияВидЗанятостиОтбор = ВидСравненияКомпоновкиДанных.ВСписке;
			Иначе
				ВидСравненияВидЗанятостиОтбор = ВидСравненияКомпоновкиДанных.Равно;
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
				Список,
				"ВидЗанятости",
				ВидЗанятостиОтбор,
				ВидСравненияВидЗанятостиОтбор,
				,
				Истина);
			Параметры.Отбор.Удалить("ВидЗанятости");	
		
	КонецЕсли;	
	ВидДоговораОтбор = Неопределено;
	Если Параметры.Отбор.Свойство("ВидДоговора", ВидДоговораОтбор) Тогда
		
		    Если ТипЗнч(ВидДоговораОтбор) = Тип("СписокЗначений") Тогда
				ВидСравненияВидЗанятостиОтбор = ВидСравненияКомпоновкиДанных.ВСписке;
			Иначе
				ВидСравненияВидЗанятостиОтбор = ВидСравненияКомпоновкиДанных.Равно;
			КонецЕсли;
