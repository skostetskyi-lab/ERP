	...
	Для Каждого СтрокаТЧ Из ШаблоныКодовКартЛояльности Цикл
		...
		Если ТипКарты = Перечисления.ТипыКарт.Штриховая
			...
			Если СтрокаТЧ.КонецДиапазонаШтрихкода < СтрокаТЧ.НачалоДиапазонаШтрихкода Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке %1 списка ""Шаблоны кодов карт лояльности"" нижняя граница диапазона штрихкода превышает верхнюю границу дипазона штрихкода.';uk='У рядку %1 списку ""Шаблони кодів карт лояльності"" нижня межа діапазону штрихкоду перевищує верхню межу дипазона штрихкоду.'"), СтрокаТЧ.НомерСтроки),
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ШаблоныКодовКартЛояльности", СтрокаТЧ.НомерСтроки, "НачалоДиапазонаШтрихкода"),
					,
					Отказ);
