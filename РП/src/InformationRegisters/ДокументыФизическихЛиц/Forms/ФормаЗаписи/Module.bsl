...
&НаКлиенте
Процедура УправлениеСоставомПолей()
	Если Запись.ВидДокумента = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыДокументовФизическихЛиц.ВоенныйБилет") Тогда
		Элементы.ДатаВремяФормирования.Видимость = Истина;
	Иначе	
        Элементы.ДатаВремяФормирования.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры
